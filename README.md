...
**Steps 0–36 completati · 655 test verdi · clippy pulito · differential 37.835 casi a 0 mismatch.**
...
| 36 | **preg backref/lookaround** — auto-fallback `regex`→`fancy-regex` (`enum Engine`): backref, lookaround, atomic/possessive, `(?R)`/conditional/`\K`/`\G`. Scope-out: subroutine `(?1)`, control verb `(*SKIP)`, callout. Corpus `ext/pcre` 38→41 pass | ✅ |
