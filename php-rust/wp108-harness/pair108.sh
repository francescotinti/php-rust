#!/bin/bash
# pair108.sh — S-100: collaudo WordPress full+media stessa-sera + coppia peak,
# versione BIMODALE del contratto di modo (A-KL-101-6). USO: pair108.sh <off|on>
...
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp108-harness"
...
}
{
  ...
  echo "epoch=$(date +%s)"
} > "$OUT/pair108.identity"
"$GUARD" backup-wipe >> "$OUT/progress.txt" 2>&1 || { echo "rc=4 guard-backup" > "$OUT/pair108.done"; exit 4; }
cd "$WPDEV" || exit 2
...
step "pair108 modo=$MODE DONE gate_void=$GATE_VOID"
echo "rc=$GATE_VOID modo=$MODE $(date +%T)" > "$OUT/pair108.done"
...
{ echo "pair108-ratios modo=$MODE: rapporti dai .time (output macchina, non prosa)"
  echo "grade=VERDICT  # derivazione meccanica riproducibile dai raw"
  echo "formato=ascii-nudo"
  ratios
} > "$H/pair108-ratios-$MODE.out"
