...
| tokenizer | 2 / 6 | — | `token_get_all`/`token_name`/`PhpToken` + 152 `T_*` (mago lexer); byte-identical on normal code + interpolation/heredoc + error tokens + TOKEN_PARSE (semi-reserved reclassification, fixed lexer-error `ParseError`s, `->__halt_compiler` re-lex, non-canonical-cast deprecations) (42/49 official); bison/yacc syntax messages deferred |
...
| zlib | 29 / 30 | 97% | byte-identical via system zlib (libz-sys FFI, PHP exact deflate params); gz streams, compress.zlib:// across the file builtins, incremental contexts with reuse+dictionaries (suite 104/114); only ob_gzhandler + stream filters left |
