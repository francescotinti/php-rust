...
| tokenizer | 2 / 6 | — | `token_get_all`/`token_name`/`PhpToken` + 152 `T_*` (mago lexer); byte-identical on normal code + interpolation/heredoc + error tokens + TOKEN_PARSE (semi-reserved reclassification, fixed lexer-error `ParseError`s, `->__halt_compiler` re-lex) (41/49 official); bison/yacc syntax messages deferred |
