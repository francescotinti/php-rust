#!/bin/bash
# s120-census-re1.sh — MECCANISMO L-RE1 (criterio s120-criterio-re1.md p.2):
# build census (HEAD + census-clite.patch wp119 + re1.patch) nel target
...
git apply "$H/re1.patch" || { git checkout -- crates/; fail "re1.patch NON applica sopra il census"; }
...

{
  ...
  echo "patches=census-clite(wp119)+re1"
  echo "epoch=$(date +%s)"
} > "$OUT/identity.txt"
