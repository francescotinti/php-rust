#!/bin/bash
# s120-pair-intercal.sh — coppia WP N=2 per modo a GAMBE INTERCALATE
# (az. rev. S-119 §1: off1 → on1 → off2 → on2, stessa sera), ricetta pair109
# INVARIATA (si riusa lo script collaudato; qui solo orchestrazione + archivio
# per gamba + rapporti CROSS-ORACLE dai .time). Pin misurato = release/phpr
# (s120) — deviazione dal «sul pin s119» di NEXT_SESSION DICHIARATA nel
# session file (il pin s119 è superato stasera; l'oracle si campiona N=4).
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp120-harness"
...
print "cross-ratios S-120 (full master cpu = user+sys; 4 gambe intercalate off1 on1 off2 on2)\n";
