#!/bin/bash
# s124-ab.sh <BPATH> <BEXP8> <TAG> <R> <EXTRA_MAX> <cat...> — criterio
# s124-criterio-phpstr.md p.4: A/B ordine ALTERNATO per coppia (dispari A->B,
# pari B->A), timer ucpu.py µs, file SCALATI wp123 (unico perimetro bande v2),
# floor med3 per binario, parità output. TSV: cat N i tA tB fA fB ord. rc su FILE.
set -u
...
[ "$(shasum -a 256 "$A" | cut -c1-8)" = "885d2c64" ] || { echo "A != pin — STOP" | tee -a "$VERD"; echo 1 > "$RC"; exit 1; }
[ "$(shasum -a 256 "$BB" | cut -c1-8)" = "$BEXP" ]   || { echo "B != $BEXP — STOP" | tee -a "$VERD"; echo 1 > "$RC"; exit 1; }
echo "== s124-ab $TAG: A=885d2c64 B=$BEXP R=$RBASE extra<=$EXTRA_MAX cats=[$CATS] ordine ALTERNATO ==" >> "$VERD"
