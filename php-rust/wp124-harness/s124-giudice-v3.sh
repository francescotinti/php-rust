#!/bin/bash
# s124-giudice-v3.sh <TAG> <TARGET> — criterio s124-criterio-phpstr.md p.5-6:
# v3 = v2 con BSTOR e ZAVORRA RITIRATI (az. rev. S-123 #3): SOGLIA_LAYOUT v2
# unica fonte per le guardie, insieme a 2×spread_A e 4×quanto. Legge
# FAIL-CLOSED wp123-harness/metro-out/layout-bande-v2.txt e il TSV di
# s124-ab.sh. rc in metro-out/giudice-<TAG>.rc:
...
BANDE="$H/../wp123-harness/metro-out/layout-bande-v2.txt"
VERD="$H/s124-$TAG-verdetto.out"
...
# v3 (az. rev. S-123 #3): BSTOR e ZAVORRA RITIRATI — le soglie old-regime
# (timer 10 ms, N originali, ordine fisso) non parlano nel metro sanato.
FLOOR_PROMO = 4.00
...
for c in cats:
    ...
    if c == target:
        s_ref = round(max(SL[c], 2*sa, 4*quanto), 2)
        s_pro = round(max(FLOOR_PROMO, SL[c], 2*sa, 4*quanto), 2)
        ...
        soglia = round(-max(2*sa, SL[c], 4*quanto), 2)
        sf = dmed < soglia
...
print(f"VERDETTO_S124_{tag} = {label}")
print(f"RCV={rc}")
PY
...
{
  echo ""
  echo "GIUDICE S-124 $TAG bersaglio=$TARGET (rc=$R da metro-out/giudice-$TAG.rc):"
  sed '/^RCV=/d' "$OUT/giudice-$TAG.txt"
} >> "$VERD"
exit "$R"
