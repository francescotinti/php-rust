#!/bin/bash
# s181-pair.sh <tentativo> — coppia full/media WP @ pin s181 (DOVUTA: pin nuovo, promozione L-CR1 S-181).
# COPIA DICHIARATA di wp180-harness/s180-pair.sh (manifest s181-pair-copia.diff) coi SOLI adattamenti:
# nomi s181/pair181/t22; harness wp181; pin s181 = PIN_ATTESO/SRV_ATTESO dagli stash (s181-lancio-coppia.sh);
# mediane storiche con t21=1,744 (banda giudizio INVARIATA [1,738;1,799]); attesa di leva nel messaggio GIU': L-CR1
# direzione ≤0 SOTTO-risoluzione. Tutto il resto INVARIATO (testo S-180 conservato sotto).
# s175-pair.sh <tentativo> — coppia full/media WP @ pin s175 (DOVUTA: pin nuovo, promozione Sweep-in-op C S-175).
# COPIA DICHIARATA di wp172-harness/s173-pair.sh (manifest s175-pair-copia.diff) coi SOLI adattamenti:
# nomi s175/pair181/t22; harness wp174; pin s175 = phpr 5de14d6856d760a8 + server PLACEHOLDER (inserito DOPO
# pin-server.sh, dichiarazione nel criterio); mediane storiche con t19=1,772 (banda giudizio INVARIATA [1,738;1,799]);
# attesa di leva nel messaggio GIU': Sweep-in-op (B+C): direzione ≤0, SOTTO-risoluzione (quota «op in place + Sweep»
# e for canonici in WP non censita). Tutto il resto INVARIATO (testo S-173 conservato sotto per storia).
# s173-pair.sh <tentativo> — S-164 p.1a: coppia full/media WP @ pin s173 (DOVUTA: pin nuovo)
# criteri s132/s146/s148 EREDITATI INVARIATI + s181-criterio-pair-t22.md
# (GIUDIZIO CANONICO a MEDIANA per finestra vs t1=1,799 t2=1,738 t3=1,789 t4=1,781 t5=1,757 t6=1,771 t7=1,795 t8=1,754 t9=1,767 t10=1,760 t11=1,765 t12=1,767 t13=1,763 t14=1,761 t15=1,746 t16=1,749 t17=1,776 t18=1,769 —
...
# EMENDE az.rev. S-157 #3-#4 EREDITATE: header del verdetto coi pin dall'HASH
# MISURATO + sentinella language-server registrata nel verdetto (inizio e fine).
...
PIN_ATTESO="${PIN_ATTESO:-PLACEHOLDER-PIN-S181}"
SRV_ATTESO="${SRV_ATTESO:-PLACEHOLDER-SRV-S181}"
...
step "sentinella LS inizio: ${LS_START:-nessuno}"
...
python3 - "$OUT" "$T" "$PIN" "$SRV" > "$VERD" <<'PY'
import re, sys, statistics, os
out, T = sys.argv[1], sys.argv[2]
PINH, SRVH = sys.argv[3], sys.argv[4]  # az.rev. S-157 #3: identita' dall'HASH MISURATO
def t(f):
    d = {}
...
print(f"== s181 coppia WP full+media sul pin s181 MISURATO phpr={PINH} server={SRVH} tentativo={T} (criteri s132/s146/s148 EREDITATI + s181-criterio-pair-t22.md: giudizio a MEDIANA per finestra, az.rev.3 S-149) ==")
print(f"grade=VERDICT  # derivazione meccanica dai .time; rc autoritativo = pair-out/pair181-{T}.done")
...
for cfg in ("on",):
    ...
    if cfg == "on":
        # s181-criterio-pair-t22.md p.2 (az.rev.3 S-149): GIUDIZIO CANONICO a
        # MEDIANA per finestra vs mediane storiche t1=1,799 t2=1,738 t3=1,789 t4=1,781 t5=1,757 t6=1,771 t7=1,795 t8=1,754 t9=1,767 t10=1,760 t11=1,765 t12=1,767 t13=1,763 t14=1,761 t15=1,746 t16=1,749 t17=1,776 t18=1,769 t19=1,772 t20=1,765 t21=1,744;
        # la banda-unione resta SOLO companion descrittivo.
        MED_LO, MED_HI = 1.738, 1.799
        med_t22 = statistics.median(props)
        print(f"mediana_t22={med_t22:.3f} (N={len(props)} coppie proprie ON PULITE; mediane storiche t1=1.799 t2=1.738 t3=1.789 t4=1.781 t5=1.757 t6=1.771 t7=1.795 t8=1.754 t9=1.767 t10=1.760 t11=1.765 t12=1.767 t13=1.763 t14=1.761 t15=1.746 t16=1.749 t17=1.776 t18=1.769 t19=1.772 t20=1.765 t21=1.744)")
        if MED_LO <= med_t22 <= MED_HI:
            print(f"giudizio_MEDIANA (canonico, criterio t22 p.2): COMPATIBILE (mediana in [{MED_LO}; {MED_HI}]) — nessun claim")
        elif med_t22 < MED_LO:
            print(f"giudizio_MEDIANA (canonico, criterio t22 p.2): direzione GIU' SEGNALATA (mediana {med_t22:.3f} < {MED_LO}) — attesa: L-CR1 Call/Ret magri (calls-dq D=[+8,17;+8,50] ns/chiamata): direzione ≤0 SOTTO-risoluzione (quota chiamate utente ad arità esatta nel workload WP non censita) — lettura")
        else:
            print(f"giudizio_MEDIANA (canonico, criterio t22 p.2): REGRESSIONE SEGNALATA (mediana {med_t22:.3f} > {MED_HI}) — indagine PRIMA di ogni altra leva")
