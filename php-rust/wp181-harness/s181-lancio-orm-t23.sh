#!/bin/bash
# s181-lancio-orm.sh — catena p.1: attende pair181-t23.done; SOLO se rc=0
# lancia s176-orm-coppia.sh (MAPPA_SP dedicato APFS). Un pair fallito NON
# fa partire l'ORM: la sessione istruisce prima.
# COPIA DICHIARATA di s172-lancio-orm.sh (manifest s181-lancio-orm-copia.diff): soli nomi s181/t23/pair181.
set -u
...
PD="$H/pair-out/pair181-t23.done"
echo "$(date '+%F %T') attesa $PD" >> "$LOG"
while [ ! -e "$PD" ]; do sleep 120; done
if ! grep -q '^rc=0' "$PD"; then
  echo "$(date '+%F %T') pair t23 NON rc=0 ($(cat "$PD")) — ORM NON lanciato" >> "$LOG"
  exit 5
fi
sleep 60
