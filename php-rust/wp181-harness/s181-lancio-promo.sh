#!/bin/bash
# s181-lancio-promo.sh — lanciatore DETACHED della catena di promozione s181 (s181-promozione.sh).
...
H="$SRC/wp181-harness"; O="$H/promo-out"; mkdir -p "$O"
LOG="$O/lancio-promo.log"; LRC="$O/lancio-promo.rc"; LDONE="$O/lancio-promo.done"
export PROMO_SP="/private/tmp/promo-s181-sp"
rm -f "$LRC" "$LDONE"
l(){ echo "$(date '+%F %T') $*" >> "$LOG"; }
sentinelle(){
