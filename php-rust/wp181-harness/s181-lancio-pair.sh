...
# finestra quieta (uptime + top nel log) e lancia s181-pair.sh t22.
# COPIA DICHIARATA di s172-lancio-pair.sh (manifest s181-lancio-pair-copia.diff):
# soli nomi s181/t22; il resto INVARIATO (blocco anti-flare EREDITATO).
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp181-harness"
LOG="$H/pair-out/lancio-t22.log"; mkdir -p "$H/pair-out"
...
Q="$H/pair-out/quiet-decl-t22.txt"
{ echo "== dichiarazione finestra quieta t22 $(date '+%F %T') =="
  uptime
  top -l 1 -n 8 -o cpu -stats pid,cpu,command 2>/dev/null | tail -12
} > "$Q" 2>&1
echo "$(date '+%F %T') quiete raggiunta — dichiarazione in quiet-decl-t22.txt — lancio pair t22" >> "$LOG"
exec /bin/bash "$H/s181-pair.sh" t22
