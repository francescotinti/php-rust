...
5. **Dente loc +174** (loc_dente.rs 74-78): legittimo e dichiarato, ma ~45 righe sono commenti-leva e 14 la firma a 12 parametri di `binary_scsc_dst_slow` (run.rs 407-420): una struct/`&Op` avrebbe contenuto la salita. Residuo minore.
6. **fx-sl1-div congela le divergenze §3.11/§3.13** (undef-dst senza warning, riga dei diag accodati): pin==stash s166 byte-id è giusto per la leva, ma la fixture non deve diventare golden: resta un contratto «invariante», non un atteso.
