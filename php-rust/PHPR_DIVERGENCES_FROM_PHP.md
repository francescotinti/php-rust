...
>
> **Ultima verifica sul campo: 2026-08-03** — ogni voce rimasta è stata
> ri-eseguita contro l'oracle 8.5.7 in questa data. Rimosse in quella
> revisione: `get_cfg_var`/`ini_get_all`, `timezone_identifiers_list`,
> `memory_limit` (§3.5), riferimenti condivisi `r:`/`R:` e `&` in
> `var_dump($_SESSION)` e flusso `-d` (§3.6), `$flags` dei value-sort e
> diagnostica string-offset READ e deprecation incr/decr (§3.7), ordering
> dei side-effect di autoload (§3.8). Aggiunte: §1.7 (rifiuto a parse-time)
> e §3.10 (`mt_rand` oltre i 32 bit — **era pubblicata come invariante**).
...
- **Residuo minimo rimasto**:
  (a) `sprintf`/`printf` — **la divergenza si è INVERTITA** (verificato
      2026-08-03): `%s` con un oggetto `Stringable` ora è a parità, ma `%d`
      diverge nell'altro senso — l'oracle emette `Warning: Object of class E
      could not be converted to int` e stampa `1`, phpr stampa il valore
      convertito senza warning. La radice resta la stessa (coercion
      PER-specifier non guidabile dalla builtin senza re-entrancy VM).
      **Deferito.**
...
| `preg_last_error` / `preg_last_error_msg` | Presenti ma lo stato d'errore non è popolato: su backtrack-limit l'oracle dà `2`/`Backtrack limit exhausted`, phpr `0`/`No error` (il caso UTF-8 `4` coincide) | Ponte allo stato d'errore dell'engine PCRE |
