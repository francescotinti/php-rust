#!/bin/bash
# s131-promozione.sh <cand_hash16> — gate di promozione L-OL1-E1-KO resolve-once — S-131 (copia dichiarata di s130-promozione.sh: soli tag s131 e path wp131; inventario fixture per NOME già emendato)
# (criterio s131-criterio-e1ko.md p.6; A/B R=5).
...
VERD="$H/s131-promo-verdetto.out"
CAND_EXP="${1:?uso: s131-promozione.sh <cand_hash16>}"
...
"$SRC/scripts/pin-phpr.sh" s131 > "$OUT/pin.log" 2>&1
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s131.out" 2>&1
note "promozione micro pin s131: $(grep -E '^rapporto_' "$OUT/micro-pin-s131.out" | tr '\n' ' ')"
...
"$SRC/scripts/pin-server.sh" s131 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb — il file rc che QUESTO script scrive): pin s131 = $H2"
exit 0
