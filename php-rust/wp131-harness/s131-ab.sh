#!/bin/bash
# s131-ab.sh <BPATH> <BEXP8> <TAG> <R> — LEVA E1-KO (criterio in
# s131-criterio-e1ko.md, commit PRIMA di ogni run). COPIA DICHIARATA di
# wp130-harness/s130-ab.sh coi SOLI adattamenti del criterio p.5:
#  (a) A = stash pin s130 (phpr-s130, 0fdf1c49);
#  (b) bande micro-ORM FONDATE sulle 7 gambe B committate S-130 (stesso
#      binario che qui e' A): objalloc 6.7 / objchurn 20.0 / objmap 13.3;
#  (c) quiescenza gate SEPARATO eseguito QUI in testa (file proprio) e
#      HEADER che ne stampa file+valore (az.rev. S-130 #3).
# Tutto il resto INVARIATO: giudice objdatains, coppie ALTERNATE, user CPU
# /usr/bin/time -p, floor med3 per binario, rumore trimmed drop-1 simmetrico,
# guardie SOLO-REGRESSIONE, sei storiche su SL committate.
...
[ "$(shasum -a 256 "$A" | cut -c1-8)" = "0fdf1c49" ] || { echo "A != pin s130 — STOP" | tee -a "$VERD"; echo 1 > "$RC"; exit 1; }
...
{
echo "== s131-ab $TAG: A=0fdf1c49(pin s130) B=$BEXP R=$R ordine ALTERNATO; giudice=objdatains; rumore trimmed drop-1 simmetrico; bande fondate objalloc 6.7/objchurn 20.0/objmap 13.3; quiescenza rc=$QRC (file: ab-out/quiesce-$TAG.rc); rc autoritativo = ab-out/$TAG.rc =="
...
# (criterio s131-e1ko p.4: range drop-1 delle 7 gambe B committate S-130).
SL = {"arith":0.94,"prop":0.80,"calls":0.73,"str":2.89,"arr":2.49,"re":4.46}
BAND = {"objmap":13.3,"objalloc":6.7,"objchurn":20.0}
def med(v):
    s = sorted(v); n = len(s)
