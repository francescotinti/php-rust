#!/bin/bash
# s131-submicro.sh — rimisura micro-ORM sul pin s131 POST-E1-KO.
# COPIA DICHIARATA di wp127-harness/s127-submicro.sh: soli cambi = pin atteso
# s131 (ff66cb846e6cd439), OUT/VERD in wp131-harness. Metodo INVARIATO
...
VERD="$H/s131-submicro-verdetto.out"
...
rm -f "$OUT/micro.done"
[ "$(shasum -a 256 "$PHPR" | cut -c1-16)" = "ff66cb846e6cd439" ] || { echo "rc=9 pin!=s131" > "$OUT/micro.done"; exit 9; }
...
{
echo "== s131 micro-ORM POST-E1-KO sul pin s131 (metodo s127-criterio-submicro.md INVARIATO) =="
