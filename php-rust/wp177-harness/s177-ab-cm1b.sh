#!/bin/bash
# s177-ab-cm1b.sh — RERUN (finestra cm1 contaminata) con EMENDA E1 (gate banda same-binary) — A/B della leva COMPOSTA «flag gc-idle + L-CM1» (criterio s177-criterio-cm1.md p.3-5): COPIA
...
# Uso: s177-ab-cm1b.sh <APATH> <AEXP8> <ZPATH> <ZEXP8> <BPATH> <BEXP8> <CPATH> <CEXP8> <TAG> [R] [PREV_DQ_A] [PREV_PROP_A]
set -u
...
# EMENDA E1 (S-177, finestra cm1 CONTAMINATA da app utente: A 54,67 vs PREV 36,20 same-binary): banda same-binary |A−PREV| > 4 su un giudice ⇒ nessun verdetto (rc=8)
BANDp = abs(med(pA)-prevpd); BANDg = abs(med(gA)-prevdq)
if BANDp > 4.0 or BANDg > 4.0:
    print(f"BANDA same-binary FUORI: |A−PREV| prop {BANDp:.2f} / arith {BANDg:.2f} > 4 ⇒ finestra CONTAMINATA (pesi esterni): nessun verdetto (rc=8, emenda E1 S-177)"); rc = 8
