# REPORT_GAP_46 — gap perf oracle↔phpr, cumulativo fino a WP-46
...
| WP-46 | 59,39/20,82 = **2,85×** ⚠️ **REGRESSIONE +7,0% TENUTA per direttiva no-revert** (old dd148a0 stesso-giorno 55,50 = 2,67×; 6/6 round new>old, oracle 20,79-20,85; costo = 11 collect × classify su ~726k root VIVI + rooting container nel note-path, ripagati da freed 543 = nulla) — in cambio: **parità funzionale migliore di sempre** (corpus 1447→1421, famiglia gc 36→14, tutte le 18 probe gc oracle byte-id) | 4,55/0,37GB = **12,3×** peak fisico (old di giornata 4,43-4,46 = 12,0×; +2,5% = buffer ctr + walks) — **il bersaglio ~3G NON è caduto: collector operativo ma i root notati sono VIVI (mem-census: arr 3,113M/1,9G e str 23,8M/1,29G INVARIATI alla cifra). La diagnosi WP-45 "cicli irraggiungibili" va ri-attribuita: WP-47 = owner-tracer + root-walk esteso (FramePool, tabelle VM)** | full-suite: [riga aggiornata a fine run WP-46] | [idem] |
