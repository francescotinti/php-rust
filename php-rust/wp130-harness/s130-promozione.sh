#!/bin/bash
# s130-promozione.sh <cand_hash16> — gate di promozione L-OL1-F4 prelude-gate — S-130 (copia dichiarata di s129-promozione.sh: soli tag s130 e path wp130)
# (criterio s129-criterio-f4.md p.6; A/B R=5 in corso).
# Ordine REGOLE §6 (leva GIÀ committata @ 89e59ea (rientro F4, revert del revert)): build ricetta (riproduce il
...
VERD="$H/s130-promo-verdetto.out"
CAND_EXP="${1:?uso: s130-promozione.sh <cand_hash16>}"
...
"$SRC/scripts/pin-phpr.sh" s130 > "$OUT/pin.log" 2>&1
...
"$SRC/scripts/pin-server.sh" s130 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb — il file rc che QUESTO script scrive): pin s130 = $H2"
exit 0
