#!/bin/bash
# s130-submicro.sh — rimisura micro-ORM sul pin s130 POST-F4 (NEXT_SESSION §S-130 p.3).
# COPIA DICHIARATA di wp127-harness/s127-submicro.sh: soli cambi = pin atteso
# s130 (0fdf1c49b16c24ba), OUT/VERD in wp130-harness. Metodo INVARIATO
...
VERD="$H/s130-submicro-verdetto.out"
...
rm -f "$OUT/micro.done"
[ "$(shasum -a 256 "$PHPR" | cut -c1-16)" = "0fdf1c49b16c24ba" ] || { echo "rc=9 pin!=s130" > "$OUT/micro.done"; exit 9; }
...
{
echo "== s130 micro-ORM POST-F4 sul pin s130 (metodo s127-criterio-submicro.md INVARIATO) =="
