#!/bin/bash
# pair100.sh — S-100: collaudo WordPress full+media stessa-sera + coppia peak,
# versione BIMODALE del contratto di modo (A-KL-101-6). USO: pair100.sh <off|on>
...
}
{
  ...
  echo "epoch=$(date +%s)"
} > "$OUT/pair100.identity"
"$GUARD" backup-wipe >> "$OUT/progress.txt" 2>&1 || { echo "rc=4 guard-backup" > "$OUT/pair100.done"; exit 4; }
cd "$WPDEV" || exit 2
...
step "pair100 modo=$MODE DONE gate_void=$GATE_VOID"
echo "rc=$GATE_VOID modo=$MODE $(date +%T)" > "$OUT/pair100.done"
...
{ echo "pair100-ratios modo=$MODE: rapporti dai .time (output macchina, non prosa)"
  echo "grade=VERDICT  # derivazione meccanica riproducibile dai raw"
  echo "formato=ascii-nudo"
  ratios
} > "$H/pair100-ratios-$MODE.out"
