...
VERD="$H/s174-$TAG-verdetto.out"; RC="$OUT/$TAG.rc"
...
print(f"GUARDIA prop-dq (sola regressione; D = direzione, magnitudine NON ripartita — attesa dichiarata [2;6], non giudicata): oracle={med(pO):.2f} ns/iter; A/oracle={med(pA)/med(pO):.2f}× B/oracle={med(pB)/med(pO):.2f}× C/oracle={med(pC)/med(pO):.2f}×")
_, _, _, _, regGB = stat("GUARDIA prop-dq", pA, pB, "B", "A", SLp)
_, _, _, _, regGC = stat("GUARDIA prop-dq", pA, pC, "C", "A", SLp)
if rc == 8:
...
if not ok:
    ...
    if nom:
        print(f"PROMOZIONE (p.4: braccio con D maggiore, nominato da ENTRAMBI gli stimatori, guardia senza regressione): AMMESSA — braccio {lab} (D={D:+.2f})")
    else:
