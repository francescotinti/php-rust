...
"$SRC/scripts/pin-phpr.sh" s174 > "$OUT/pin.log" 2>&1
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s174.out" 2>&1
note "promozione micro pin s174: $(grep -E '^rapporto_' "$OUT/micro-pin-s174.out" | tr '\n' ' ')"
...
note "conferma post-pin arith-dq (pin s174 vs stash s173): $CONF (attesa: segno +, D nell'intorno del D_dq del braccio promosso nell'A/B; rumore > attesa/2 ⇒ SOLO SEGNO)"
...
"$SRC/scripts/pin-server.sh" s174 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb): pin s174 = $H2"
exit 0
