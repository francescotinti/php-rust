...
# rc autoritativo = ab-out/<TAG>.rc; verdetto s168-<TAG>-verdetto.out.
set -u
...
VERD="$H/s168-$TAG-verdetto.out"; RC="$OUT/$TAG.rc"
...
D = med(dqA)-med(dqB); noise = max(dr1(dqA), dr1(dqB)); thr = max(4.0, noise, 0.94)
