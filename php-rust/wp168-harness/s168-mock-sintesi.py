...
R = {t: parse(t) for t in ("m0", "m1", "m2", "m3", "m123", "m4b")}
for t, r in R.items():
...
if len(mocks) == 3:
    ...
    if m4 and "err" not in m4:
        ...
        tot = e2 + sum_named + d4
        out.append(f"m4b dispatch Sweep/iter (criterio p.8-9): D={m4['D']:+.2f} soglia={m4['thr']:.2f} -> {'NOMINATO' if d4 else 'non nominato (0)'}; Σ mock CON m4 = {sum_named + d4:.2f} -> kill p.4 {'SCATTA' if sum_named + d4 < 10 else 'NON scatta'}; CHIUSURA CON m4 = ({e2:.2f}+{sum_named:.2f}+{d4:.2f})/{dq:.2f} = {tot/dq*100:.1f}%")
        resid = dq - tot
