//! S-137 sonda ripartizione eccedenza FD1 (criterio s137-criterio-sonda.md) —
//! build emendata nel WORKTREE s136, MAI nel pin. UN segmento attivo per run
//! (`PHPR_TP=<seg>`); inattivo = un branch + load. Contatori hit/miss del
//! fast path attivi con `PHPR_TPC=1` (run SEPARATO dal tempo, criterio p.4).
//! Dump atexit su `PHPR_TP_OUT` (ns accumulati + conteggio span + hit/miss).
...
fn counting() -> bool {
    ...
    *C.get_or_init(|| std::env::var("PHPR_TPC").is_ok_and(|v| v == "1"))
}
...
/// Contatore HIT del fast path FD1 (solo con PHPR_TPC=1).
#[inline(always)]
pub fn hit() {
    if !counting() {
        return;
...
/// Contatore MISS del fast path FD1 (solo con PHPR_TPC=1).
#[inline(always)]
pub fn miss() {
    if !counting() {
        return;
