#!/bin/bash
# s158-ab-refl2.sh <BPATH> <BEXP8> <TAG> <R> <AEXP8> [DSMOKE_REFL] — LEVA
# L-RF2 tranche-2 slice __reflect_* (criterio s158-criterio-refl2.md).
# COPIA DICHIARATA di s157-ab-al1.sh coi SOLI adattamenti del criterio:
#  (a) GIUDICE: refl (m-refl.php NUOVO wp158-harness, N=10000000 DICHIARATO;
#      n_fixed lo pinna comunque);
#  (b) UB-alloc FALSIFICABILE = 13.8 (2 alloc/iter x miheap 6.9);
#  (c) guardie: missload (presidio L-AL1) + hostargs + backtrace24 + obj* +
#      le sei; comparatore STRETTO (az.rev. S-156 #1): morde sse D < -soglia;
#  (d) A = GEMELLO RICOSTRUITO sul tree s157, hash atteso <AEXP8> PASSATO al
#      run (arbitrato a contenuto nel verbale s158-gemelloA-identita.out);
#      header con hash MISURATI dei bracci (az.rev. S-157 #3);
#  (e) sentinella language-server nel verdetto (az.rev. S-157 #4);
#  (f) lock/verdetto/tag s158.
set -u
...
A="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s158-gemelloA"
...
if ! grep -qi "s-158\|s158" "$LOCK" 2>/dev/null; then
...
# s158-gemelloA-identita.out). Gli hash MISURATI finiscono nell'header.
...
src_of() { case "$1" in refl|missload|hostargs|backtrace) echo "$H";; objalloc|objchurn|objmap|objdatains|objallocni|objdropdef) echo "$M";; *) echo "$M97";; esac; }
php_of() { case "$1" in refl) echo "m-refl.php";; missload) echo "m-missload.php";; hostargs) echo "m-hostargs.php";; backtrace) echo "m-backtrace24.php";; *) echo "$1.php";; esac; }
n_fixed() { case "$1" in refl) echo 10000000;; missload) echo 10000000;; hostargs) echo 10000000;; backtrace) echo 2400000;; arith) echo 150000000;; prop) echo 90000000;; calls) echo 60000000;; str) echo 28000000;; arr) echo 30000000;; re) echo 12000000;; *) echo "";; esac; }
CATS_JUDGE="refl"
CATS_GUARD="missload hostargs backtrace objdropdef objchurn objdatains objalloc objallocni objmap arith prop calls str arr re"

{
echo "== s158-ab-refl2 $TAG: A=$AM MISURATO (gemello RICOSTRUITO tree s157) B=$BM MISURATO R=$R ordine ALTERNATO; giudice=refl N=10000000 dichiarato; soglia=max(4, drop-1); UB-alloc falsificabile=13.8 (2 alloc/iter x miheap 6.9); guardie missload(N=10M) + hostargs(N=10M) + backtrace24(N=2,4M) + obj* + sei, comparatore STRETTO; quiescenza rc=$QRC; rc autoritativo = ab-out/$TAG.rc =="
...
dsmoke = {}
if len(sys.argv) > 3 and sys.argv[3]: dsmoke["refl"] = float(sys.argv[3])
UB = 13.8            # criterio refl2 p.4: 2 alloc/iter x miheap 6.9
rows = {}
for l in open(tsv):
    t = l.rstrip("\n").split("\t")
