#!/bin/bash
# s128-pair-intercal.sh — S-128 p.1: rimisura full/media WP sul pin s127b,
# coppia bimodale N=2 per modo a gambe INTERCALATE off1→on1→off2→on2
# (criterio: s128-criterio-pair.md, committato PRIMA di questo codice —
# az.rev. S-127 #5). Ricetta pair109 INVARIATA (script collaudato riusato);
# qui solo orchestrazione + archivio per gamba + rapporti CROSS-ORACLE dai .time.
# Il verdetto cita SOLO il file rc scritto QUI: pair-out/pair-intercal.done
# (az.rev. S-127 #2).
set -u
...
print "cross-ratios S-128 (full master cpu = user+sys; 4 gambe intercalate off1 on1 off2 on2; pin s127b)\n";
print "grade=VERDICT  # derivazione meccanica dai .time; rc autoritativo = pair-out/pair-intercal.done\n";
...
step "DONE rc=$RC (fonte rc: $OUT/pair-intercal.done)"
exit "$RC"
