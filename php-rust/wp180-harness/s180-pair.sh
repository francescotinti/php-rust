...
# nomi s175/pair180/t21; harness wp174; pin s175 = phpr 5de14d6856d760a8 + server PLACEHOLDER (inserito DOPO
...
# criteri s132/s146/s148 EREDITATI INVARIATI + s180-criterio-pair-t21.md
...
PIN_ATTESO="${PIN_ATTESO:-PLACEHOLDER-PIN-S180}"
SRV_ATTESO="${SRV_ATTESO:-PLACEHOLDER-SRV-S180}"
...
print(f"== s180 coppia WP full+media sul pin s180 MISURATO phpr={PINH} server={SRVH} tentativo={T} (criteri s132/s146/s148 EREDITATI + s180-criterio-pair-t21.md: giudizio a MEDIANA per finestra, az.rev.3 S-149) ==")
print(f"grade=VERDICT  # derivazione meccanica dai .time; rc autoritativo = pair-out/pair180-{T}.done")
...
for cfg in ("on",):
    ...
    if cfg == "on":
        # s180-criterio-pair-t21.md p.2 (az.rev.3 S-149): GIUDIZIO CANONICO a
        ...
        if MED_LO <= med_t21 <= MED_HI:
            ...
            print(f"giudizio_MEDIANA (canonico, criterio t21 p.2): direzione GIU' SEGNALATA (mediana {med_t21:.3f} < {MED_LO}) — attesa: NESSUNA pre-registrata (toolchain 1.98.1 + gc-idle tenuta S-176) — lettura")
        else:
