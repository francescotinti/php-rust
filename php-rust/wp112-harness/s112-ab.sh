#!/bin/bash
# s112-ab.sh <categoria> <R> — A/B interleaved ABAB fra pin S-109 (A, stash
# = pin 92909544) e candidato H-A2 S-112 (B, binario CONSERVATO
# phpr-s112-ab-candidate: l'A/B non dipende dal churn di release/phpr).
...
B="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s112-ab-candidate"
C="${1:?categoria}"; R="${2:-5}"
...
# arr.php: il primo loop è il setup (100k); il denominatore vero è 60×100k inner.
[ "$C" = arr ] && N=6000000
