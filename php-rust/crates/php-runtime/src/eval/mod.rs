...
use php_types::{
    convert, dtoa, ClosureInfo, ClosureParam, ClosureRender, Diag, Diags, GenDriver, GenKey, GenStep, Key, Object, ObjectInfo, PhpArray,
    PhpError, PhpStr, Zval,
};

use crate::builtin::Registry;
use crate::hir::{
    ClassDecl, ClassId, Expr, ExprKind, FnDecl, Line, Place, PlaceBase, PlaceStep, Program, Slot,
};
...
// Scalar type-hint coercion moved to `crate::coerce` (Session F: the VM shares
// it to enforce parameter/return hints without depending on the evaluator).
pub(crate) use crate::coerce::{coerce_to_hint, php_type_name};
