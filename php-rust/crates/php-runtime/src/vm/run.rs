...
impl<'m> super::Vm<'m> {
    ...
    /// folded away (WP-44 v3): queue the PHP 8 "Undefined variable" warning
    /// on `Undef` — the pass folds a slot only after proving
    /// `slot_names[slot]` equals the LoadVar's name const, so the message is
    /// byte-identical — and deref-clone a Ref.
        ...
        loop {
            ...
            match op {
                ...
                Op::BinarySS { op: b, l, r } => {
                    // Raw-register fusion (WP-44 v3): bare u16 slot indices,
                    // zero runtime operand dispatch. Fast path borrows both
                    // slots into binary_fast; Undef (LoadVar warning), Ref
                    // (deref) and tag-pair misses take the owned funnel.
                    let res = 'r: {
                        {
