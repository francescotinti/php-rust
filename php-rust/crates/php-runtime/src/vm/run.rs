...
/// A direct operand of [`Op::BinaryReg`]/[`Op::CmpJmpReg`], resolved ONCE
/// (Leva B stage 2): borrowed straight from the slot file when the value is
/// plain, owned when materialisation was required (a `Const`, a `Ref` slot
/// deref, an `Undef` read). `binary_fast` runs on [`RegSrc::get`]; a miss
/// takes [`RegSrc::into_owned`] — an Rc-bump clone for a borrow, a move for
/// an owned value — so the generic funnel never re-resolves the operand.
enum RegSrc<'a> {
    B(&'a Zval),
    O(Zval),
}
...
/// Resolve a non-Stack direct operand with the semantics of the op the
/// lowering pass folded away: `Slot` is a `LoadVar` read (queue the PHP 8
/// "Undefined variable" warning on `Undef` — the pass folds a slot only
/// after proving `slot_names[slot]` equals the LoadVar's name const — and
/// deref a Ref), `Const` is a `PushConst` materialisation (ZStr refcount
/// bump). Free function so the caller can split-borrow `frames` and `diags`.
#[inline(always)]
fn reg_src<'a>(
    slots: &'a [Zval],
    ...
    match o {
        Operand::Slot(i) => {
            ...
            match &slots[i] {
                ...
                Zval::Ref(r) => RegSrc::O(r.borrow().clone()),
                v => RegSrc::B(v),
            }
        }
        Operand::Const(i) => RegSrc::O(f.consts[*i as usize].to_zval()),
        Operand::Stack | Operand::Temp(_) => unreachable!("reg_src: Stack/Temp resolved by caller"),
    }
}
        ...
        loop {
            ...
            match op {
                ...
                Op::BinaryReg { op: b, l, r, dst } => {
                    ...
                    // sourcing. Operands resolve ONCE — borrow for a plain
                    // slot, owned for Const/Ref/Undef (LoadVar/PushConst
                    // semantics, in the original order); binary_fast runs on
                    // the borrows and a miss REUSES that resolution (Rc-bump
                    // clones), never a second materialisation. The funnel is
                    // binary_value_ab, identical semantics by construction.
                    let res = if matches!(l, Operand::Stack) {
                        let (lhs, rhs) = if matches!(r, Operand::Stack) {
                            // dst-only fold: pop exactly like binary_value —
                            // rhs first.
                            ...
                            let lhs = self.frames[top].stack.pop().expect("BinaryReg lhs");
                            let rhs = reg_src(&self.frames[top].slots, func, &mut self.diags, r)
                                .into_owned();
                            (lhs, rhs)
                        };
                        self.binary_value_ab(*b, lhs, rhs)?
                        ...
                        let fast = {
                            let ls = reg_src(&self.frames[top].slots, func, &mut self.diags, l);
                            let rs = reg_src(&self.frames[top].slots, func, &mut self.diags, r);
                            match binary_fast(*b, ls.get(), rs.get()) {
                                Some(v) => Ok(v),
                                None => Err((ls.into_owned(), rs.into_owned())),
                            }
                        };
                        match fast {
                            Ok(v) => v,
                            Err((lhs, rhs)) => self.binary_value_ab(*b, lhs, rhs)?,
                        }
                    };
                    self.reg_store(top, dst, res)?;
                }
                Op::CmpJmpReg { op, l, r, addr, when } => {
                    // Register-form CmpJmp — folded shapes only (the pure
                    // stack-lhs compare keeps its monomorphic CmpJmpConst/
                    // CmpJmp handler): same compare funnel, single operand
                    // resolution, then branch.
                    debug_assert!(
                        !matches!(r, Operand::Stack),
                        "CmpJmpReg: r is never Stack (folded shapes only)"
                    );
                    let res = if matches!(l, Operand::Stack) {
                        let lhs = self.frames[top].stack.pop().expect("CmpJmpReg lhs");
                        let rhs = reg_src(&self.frames[top].slots, func, &mut self.diags, r)
                            .into_owned();
                        ...
                        let fast = {
                            let ls = reg_src(&self.frames[top].slots, func, &mut self.diags, l);
                            let rs = reg_src(&self.frames[top].slots, func, &mut self.diags, r);
                            match binary_fast(*op, ls.get(), rs.get()) {
                                Some(v) => Ok(v),
                                None => Err((ls.into_owned(), rs.into_owned())),
                            }
                        };
                        match fast {
                            Ok(v) => v,
                            Err((lhs, rhs)) => self.binary_value_ab(*op, lhs, rhs)?,
                        }
                    };
