...
/// S-77.6.5.1: Per-request setup lifecycle (WP-77.6).
    ...
    pub fn request_start(&mut self, argv: Option<&[&[u8]]>, ini_overrides: &[(Vec<u8>, Vec<u8>)]) {
        ...
        if let (Some(argv), Some(prog)) = (argv, self.main_hir) {
            seed_cli_superglobals(&mut self.superglobals, &mut self.frames[0].slots, &prog.slots, argv);
            let mut arr = PhpArray::new();
            for a in argv {
                let _ = arr.append(Zval::Str(PhpStr::new(a.to_vec())));
            }
            let slot = self.global_slot_by_name(b"argv");
            if matches!(self.frames[0].slots[slot], Zval::Undef) {
                self.frames[0].slots[slot] = Zval::Array(Rc::new(arr));
            }
            let slot = self.global_slot_by_name(b"argc");
            if matches!(self.frames[0].slots[slot], Zval::Undef) {
                self.frames[0].slots[slot] = Zval::Long(argv.len() as i64);
            }
        }
        // INI overrides and session auto-start
        self.apply_ini_overrides(ini_overrides);
        if self.ini.get_bool(b"session.auto_start") {
            let _ = self.ho_session_start(Vec::new());
        }
    }

    /// S-77.6.5.1: Per-request teardown lifecycle (WP-77.6).
    /// Handles output flushing, shutdown functions, destructors, GC, and session shutdown.
    /// Called after vm.run() and flush_diags(), before request_end().
    /// Caller must set final_flush=true before calling this (to disable error routing).
    pub fn request_shutdown(&mut self) {
        ...
        // and before returning. Under FAST_SHUTDOWN the process exit reclaims
        // everything, so the pass is skipped (zero CLI cost).
        if !FAST_SHUTDOWN.load(std::sync::atomic::Ordering::Relaxed) {
            self.break_request_cycles();
        }
        ...
        // includes, shutdown-path probes) land before the outcome is built.
        uc_log_flush();
    }
