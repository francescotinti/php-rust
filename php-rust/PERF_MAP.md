...
Aggiornata: **2026-09-22 pomeriggio (S-181)** · **PIN NUOVO s181 phpr 19a2faa83a492745 + server b2802f08c5887e77** (toolchain Rust 1.98.1)
(**S-181 = leva L-CR1 «Call/Ret magri» PROMOSSA, pin s181** (criterio wp181-harness/s181-criterio-cr1.md + emenda E3; verdetti s181-cr1{,b,c}-verdetto.out, s181-leva-build-verdetto.out, s181-promo-verdetto.out; revisione lente MISURA revisione-s181.md REGGE CON RILIEVI): bytecode del driver calls = 6 op chiamante + 3 callee (CheckArity·BinarySS·Ret) ≈ 11 ns/op vs dispatch 1,75 ⇒ corpi Call/Ret; census ORM riusato da S-176 (Ret 33,3M = 6,1 % op, CheckArity 12,9M, MethodCall→CheckArity 7,1M; tetto ≈1 % di tempo ORM dichiarato) · L-CR1 (172814ae, +32 LOC dichiarate): (a) Call fast path frame in pila prima del bind via split_at_mut, (b) ingresso a ip=1 ad arità esatta (Call fast + methodcall_fast), (c) guardia magic in Ret solo con ext · presidio: fixture bilaterale fx-cr1 (59 righe, 5 ACE) == oracle, mutante ip=1 sul cammino lento MORDE (ACE 5→2), B a parità su 6 fixture, disasm run_loop pin bl 6123 → B 6132 (+9, atteso −1: il Frame viaggia ancora per valore — meccanismo (a) da riscrivere), sp_refs −8 % · **A/B** 3 bracci (A pin s180 · Z gemello ec8d3551 · B 5ed39690) R=5, giudice NUOVO calls-dq (N=60M): cr1 [+8,17;+8,50] e cr1b [+8,33;+8,50] NOMINATE ma a guardia (un campione GoogleUpdater --wake-all), EMENDA E3 pre-registrata (updater = allarme se ≥2 campioni), **cr1c di RECORD: A 98,83 → B 90,83 ns/iter (oracle 21,67: 4,56×→4,19×), CIFRA [+8,00;+8,17]**, segni 15/15, |A−Z| ≤0,17, E1 prop-dq |A−PREV| ≤0,33; guardie prop-dq +2,40/+2,47 e arith-dq +0,72 senza regressione ma FUORI attesa |D|<1 ⇒ effetto trasversale · **promozione** (copia di s180 con HEAD+status prima della build, hk.rc nel verdetto): build 19a2faa8 ≠ B (candidato a contenuto), batteria 1748/0/2, build2 al byte, corpus 1412 zero flip, fixture chain 10/10, 20 gate byte-id (fx-cr1 nuova), E2 t8 · quiescenza t3, **micro arith 2,1 · prop 2,3 · calls 4,0 · str 4,1 · arr 2,8 · re 2,6**, conferma post-pin calls-dq +8,00 e prop-dq +2,47 (5/5) sul layout del pin (trasversale deterministico, quota non ripartita), ORM 16 nomi, hk 0E/0F, server b2802f08 · coppia DOVUTA: WP t23 mediana **1,799** = bordo superiore della banda [1,738;1,799] ⇒ giudizio canonico REGRESSIONE SEGNALATA (coppie proprie 1,735/1,783/1,799/1,799/1,809/1,814, banda_ON 0,079 vs 0,030 a t21; media user-only 2,44-2,53 vs 2,42-2,44: finestra più lenta su TUTTE le gambe, macchina in uso con carico 4-5,5; t22 abortita rc=8 per loadavg 7,9) — VOCE, non cifra · ORM sul pin s181 FINESTRA CONTAMINATA: leg1 net 6,721 · leg2 8,250 SEGNALATA dal gate ictx (phpr2 5250/s vs 188, user 43,3 s vs 34,9, sys 3,28), sentinella oracle 5,25/5,30 FUORI banda [4,84;5,04] ⇒ nessun claim, regola 4 NON giudicabile; parità ORM 16 · dbal 10 ok; dbal [7,472;7,909] (wp181-harness/s181-orm-coppia-verdetto.out) ⇒ ISTRUTTORIA in apertura di S-182 (criterio t22 p.1: «indagine PRIMA di ogni altra leva»): rerun WP t24 + ORM E3/E4 in finestra NOTTURNA quieta (carico <3, nessuna app utente); se t24 rientra in banda e ORM torna ≤7,05 ⇒ t23/ORM-s181 = contaminazione ambientale dichiarata; se persiste ⇒ reperto CONTRO L-CR1 (attesa era ≤0), istruttoria vera prima di ogni leva · incidenti: 1 (lanciatore cr1b, rc=127 senza misura) · leve: 1 promossa · sessioni senza misura: 0)

(**S-180 = ratifica S-178/S-179 (ChatGPT Astra 6) + revert L-CM1 + RI-PIN s180 sotto toolchain 1.98.1 + coppia t21/ORM in catena** (criteri wp180-harness/s180-criterio-ripin.md, s180-criterio-pair-t21.md; verdetti wp180-harness/s180-promo-verdetto.out, s180-pair-verdetto-t21.out): **fill IC slot privati = DECLASSATO a bassa priorità su misura di tempo (tetto inclusivo; readonly NON misurato)** (wp178/wp179-harness: miss ORM 4,98M = private∩RO 49 % · private∩¬RO 36 % (1,795M, 68 % ammissibili alle 7 guardie, PDOStatement 400k) · ¬priv 15 %; sonda a tempo Instant 1/16-1/64 R=3: cammino lento private¬RO ≈0,42-0,43 s inclusivi su ~37 s di suite ≈1,1 % ⇒ frequenza ≠ tempo; PHPUnit nel tarball = 11.5.56) · ratifiche: licenza PHP-3.01 · AGENTS.md mappa delle fonti · toolchain 1.96.0→1.98.1 (da qui ogni A/B è same-toolchain) · L-CM1 revertata al byte (b3e48919) · dente LOC alzato e dichiarato per gc-idle (host +1, mod +59, run +11) ⇒ CI di nuovo verde attesa · **ri-pin s180** (catena copia dichiarata S-177 + E2 + guardia disco, 34 min, rc=0): build al byte ×2, batteria 1748/0/2, corpus 1412 zero flip, fixture chain 10/10 + 19 gate byte-id, ORM 16 nomi 3484/11989 3E/13F, http-kernel 1665 0E/0F · **micro di record s180** (E2 PASS t3 campioni 100/77/97/67 %, quiescenza t1): **arith 2,2 · prop 2,4 · calls 4,5 · str 4,1 · arr 2,9 · re 2,5** (vs s175 2,4/2,6/4,6/4,1/3,0/2,5: direzione toolchain+gc-idle, NON ripartita) · prop-dq pin s180 34,20 vs stash s175 36,20 (D +2,00 5/5 rumore 0,40: sola direzione, toolchain diversa) · coppia t21: WP 1,744 COMPATIBILE (N=6, banda_ON 0,030; media 2,42-2,44) · ORM net [7,008;7,060] (leg2 >7,05 a filo: voce riaperta, regola 4 a t22; sentinella in banda; parità 16/10) · incidenti: 0 · leve: 0 (anomalia dichiarata) · sessioni senza misura: 0)

(**S-177 = leva composta prop-dq «L-CM1» a parità ma misurata in finestra CONTAMINATA (rerun cm1b in coda) + census IC miss ORM per CAUSA + CI potata** (criteri wp177-harness/s177-criterio-cm1.md, s177-criterio-census-miss.md; verdetti wp177-harness/s177-*.out): fetta 4 typed-skip ESCLUSA (classe plain: P4 prende sempre; census micro typed 0) · DISASM del pin (disasm-pin-run_loop-stats.txt): run_loop 71 745 istr, frame ≥12 KB, 11 270 [sp], 6 `blr` tutti nel prologo ⇒ ipotesi «TLV a ogni hit IC» REFUTATA; ogni hit = reload TLV dallo stack + load epoch + cmp (30 siti) · **L-CM1** = `Vm::ic_epoch` cached (get/fill con epoch esplicita) + Add-first in `long_arith_i64`, braccio C 62d1a2e3 a PARITÀ (fx-sw2-gc/sw1/sl1/sl2/sl3), disasm C blr 5, istr +287, sp_refs +839 · **A/B cm1**: rc=4 formale in finestra CONTAMINATA (A stesso binario 54,67 vs PREV 36,20, oracle 22,60 vs 14,07: +60 % uniforme, Chrome ×4 a 100 % col gate s129 PASS) ⇒ senza valore; emende E1 (gate banda same-binary >4 ⇒ rc=8) + E2 (calma CPU totale <150 %); rerun cm1b rc=8 (calma CPU mai raggiunta in 90 min) ⇒ L-CM1 SENZA verdetto, A/B da rifare in S-178 · **census miss per causa** (ramo s177-census, parità ORM/media rc=0): ORM miss 4,98M = **ic_empty 95,4 %**, ic_class 4,6 %, lazy 0; esito **private_mangled 85,0 % · readonly 59,2 %** · np_nofill 10,3 % · np_fill_typed 3,8 % · plain_fast 0,8 %; media miss 975k = ic_empty 97,5 %, private_mangled 62,5 %, np_nofill 23,3 %, plain_fast 8,9 %, dynamic 2,9 % ⇒ **perimetro S-178 (TETTO: private∩readonly ≥44 % del miss, PHPUnit 13 `readonly class` ×523 domina — rilievo 1): fill dell'IC per gli slot PRIVATI nello scope dichiarante, perimetro reale ≈ private∩¬readonly da contare per classe; readonly solo con bit RO** · CI: 40 job = replay S-174 col dente storico (batteria-FAIL attesi), coda potata a HEAD, runner in attesa del lock · incidenti: 1 (gate cieco ai pesi utente) · leve: 1 (A/B eseguito) · sessioni senza misura: 0)
