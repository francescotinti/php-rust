#!/bin/bash
# s119-grado-server.sh — S-118 criterio R3 gate 2: GRADO del pin php-server
# ripinnato sotto A′ (scripts/pin-server.sh s118, ricetta SOURCE_DATE_EPOCH=0
# CARGO_INCREMENTAL=0 + profilo lto=fat/cgu=1 dal Cargo.toml di root).
# Ricetta = s109-grado-server.sh INVARIATA salvo: harness wp118, stash
# php-server-s119, gamba phpr sul binario STASHATO phpr-s117 (=pin 1656580e,
# perché la build server churna il release/).
#
# USO: s119-grado-server.sh <off|on>
#
# LETTERA A-PE-107-1 implementata:
#   - collaudo sul binario STASHATO (hash da PIN_SRV_ATTESO.txt,
#     FAIL-CLOSED); stash `php-server-s119` (ricetta axum-server);
#   - pin phpr 3b3d25e2 riverificato (PIN_PHPR_ATTESO.txt, FAIL-CLOSED —
...
ENV_TMPDIR=/tmp
ALLOWED='PATH|HOME|TMPDIR|LC_ALL|MIMALLOC_PURGE_DELAY|PHPR_REG_LOWER|S119_GRADO_REEXEC|PWD|SHLVL|_'
...
if [ "$SANE" != 1 ]; then
  if [ "${S119_GRADO_REEXEC:-0}" != 0 ]; then
    echo "REFUSE s109-grado-server: ambiente ancora sporco dopo il re-exec — nomi extra: [${EXTRA:-none}]"; exit 65
  fi
  exec /usr/bin/env -i \
    PATH="$ENV_PATH" HOME="$ENV_HOME" TMPDIR="$ENV_TMPDIR" LC_ALL=C \
    MIMALLOC_PURGE_DELAY=0 PHPR_REG_LOWER="$REG" S119_GRADO_REEXEC=1 \
    /bin/bash "$0" ${@+"$@"}
fi
unset S119_GRADO_REEXEC

# ---- pin e strumenti ----
REPO="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust"
H="$REPO/wp119-harness"
...
# lettera A-PE-107-1: il collaudo gira sul binario STASHATO (pin s119, ricetta)
PHPSRV="/Volumes/Extreme Pro/Claude/phpr-old-target/release/php-server-s119"
PHPR="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s119"  # S-109: gamba phpr sul binario STASHATO del pin (la build server churna il release/)
...
mkdir -p "$OUT"
step() { echo "== $(date +%H:%M:%S) [S119-GRADO-$MODE] $1" | tee -a "$OUT/progress.txt"; }
FAILS=0
VOIDS=0
...
if [ "$H_SRV" != "$PIN_SRV_ATTESO" ]; then
  step "REFUSE: lo stash php-server-s119 NON è il pin da gradare"; echo "rc=65 voids=1 modo=$MODE $(date +%T)" > "$OUT/grado.done"; exit 65
fi
...
trap - EXIT
restore_uploads
step "S119-GRADO-$MODE DONE fails=$FAILS voids=$VOIDS"
