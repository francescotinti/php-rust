#!/bin/bash
# s119-treno2-build.sh — criterio s119-criterio-treno2.md p.1/4/5/8: candidato
# (HEAD + treno2.patch) nel target CANONICO con ricetta A′; admission (parità
# output 6 giudici + dump INTERO byte-id, deroga 5 dichiarata nel criterio);
...
git apply "$H/treno2.patch" || fail "patch NON applica"
...
note "candidato treno-2 = $HB (target canonico, ricetta A′; A = pin $PIN_EXP)"
...
if [ "$ADM" != 0 ]; then
  ...
  fail "admission VIOLATA (rc=1 da treno2-out/admission.rc) — candidato NON conservato, tree ripristinato"
fi
note "admission: 6/6 parità output + 6/6 dump INTERO byte-id (rc=0 da treno2-out/admission.rc)"
cp "$BIN" "$STASH/phpr-s119-treno2"
HS=$(shasum -a 256 "$STASH/phpr-s119-treno2" | cut -c1-16)
[ "$HB" = "$HS" ] || fail "stash candidato hash diverso ($HB vs $HS)"
note "candidato CONSERVATO: phpr-s119-treno2 ($HB)"
...
note "ripristino: release/phpr == pin $PIN_EXP AL BYTE; A/B pronte su stash A=phpr-s118 B=phpr-s119-treno2"
echo 0 > "$OUT/build.rc"
