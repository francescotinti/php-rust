#!/bin/bash
# s119-ab-treno2.sh smoke|full — A/B treno-2 sotto A′ (s119-criterio-treno2.md
# p.2/6): TUTTE e sei le categorie sono GUARDIE a solo-regressione (nessun
# bersaglio nei giudici); floor = mediana di 3 per binario; rc in FILE.
# Deroga 6: banda calls usata a N=3 (0,50); morso dentro banda si REGISTRA.
set -u
...
MODE="${1:?smoke|full}"
RBASE="${RBASE:-5}"
if [ "$MODE" = smoke ]; then CATS="prop calls"; RBASE=2; EXTRA_MAX=0; else CATS="arith prop calls str arr re"; EXTRA_MAX=6; fi
OUT="$H/treno2-out"; mkdir -p "$OUT"
VERD="$H/s119-treno2-verdetto.out"
TSV="$OUT/treno2-$MODE-runs.tsv"; : > "$TSV"
...
if [ "$MODE" = smoke ]; then
  SRC=$(python3 - "$TSV" <<'PY'
import sys
# smoke di GUARDIA (nessun bersaglio): early-stop solo su regressione
# marcata concorde (D <= -1.00 su tutte le coppie di una categoria).
...
echo "== VERDETTO GUARDIE A MACCHINA (banda-v2; guardie −max(2×spread; banda; 2×quanto); tie 2 dec) =="
...
print(f"VERDETTO_MICRO={'GUARDIE TENGONO (resta held-out N=3)' if ok else 'GUARDIA SFONDATA: bisezione stacca-ultimo'}")
...
{
  echo ""
  echo "VERDETTO GUARDIE A MACCHINA (s119-ab-treno2.sh full, rc=$FR da treno2-out/full-rc):"
  sed '/^RCV=/d' "$OUT/full-verdetto.txt"
} >> "$VERD"
exit "$FR"
