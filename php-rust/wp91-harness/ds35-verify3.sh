#!/bin/bash
# ds35-verify3.sh — A-DS53/A-DS54/A-DS55 (Concilio WP-92, ordine S-91.0
...
OUT="$REPO/wp91-harness/ds35-verify3.out"
ORACLE="/opt/homebrew/opt/php/bin/php"
PHPR="$HOME/Claude/php-rust-output/release/phpr"
FC=$(mktemp -d /tmp/ds35v3-filecache.XXXXXX)
...
say "== ds35-verify3 — oracle $($ORACLE -v | head -1 | awk '{print $2}') · phpr $(shasum -a 256 "$PHPR" | cut -c1-16) · git $(cd "$REPO" && git rev-parse --short HEAD) =="
...
binchk() { # A-DS47 + A-DS52: same-vector probe when the fixture is compile-fatal
  ...
  if [ "$NBIN" -ge 1 ]; then
    ...
    local PFC PSD NPRB
    PFC=$(mktemp -d /tmp/ds35v3-probecache.XXXXXX)
    PSD=$(mktemp -d /tmp/ds35v3-probesrc.XXXXXX)
...
rm -rf "$FC"
say "== ds35-verify3 DONE (45 fixtures = 18 v1 + 19 v2 + 8 v3, length-prefixed channel pins) =="
