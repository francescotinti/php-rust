#!/bin/bash
# s138-sonda-rmw-v2.sh <D_AB> — sonda attribuzione eccedenza fuori-modello RMW
# (criterio s138-criterio-sonda-rmw-v2.md, commit PRIMA del run). Probe arm-only
...
OUT="$H/sonda-rmw-v2-out"; mkdir -p "$OUT"
VERD="$H/s138-sonda-rmw-v2-verdetto.out"
RC="$OUT/sonda.rc"
DAB="${1:?D_AB da R5}"
...
{
echo "== s138 sonda attribuzione RMW (criterio s138-criterio-sonda-rmw-v2.md; D_AB=$DAB) =="
echo "probe_bin=$(shasum -a 256 "$BIN" | cut -c1-16) (build emendata, MAI pinnabile)"
echo "quiescenza: rc=$(cat "$OUT/quiesce.rc")"
FAIL=0
# gate nm v2 (criterio p.2): field_assign_fast ASSENTE in entrambi;
# field_rmw_fast nello STESSO stato (omogeneita', non assunzione di inline).
AF_P=$(nm "$BIN" | tr -d '\0' | grep -c "field_assign_fast")
AF_C=$(nm "$CAND" | tr -d '\0' | grep -c "field_assign_fast")
RW_P=$(nm "$BIN" | tr -d '\0' | grep -c "field_rmw_fast")
RW_C=$(nm "$CAND" | tr -d '\0' | grep -c "field_rmw_fast")
echo "gate_nm_v2: field_assign_fast probe=$AF_P cand=$AF_C (attesi 0/0) · field_rmw_fast probe=$RW_P cand=$RW_C (atteso STESSO stato)"
if [ "$AF_P" = 0 ] && [ "$AF_C" = 0 ] && [ "$RW_P" = "$RW_C" ]; then
  echo "gate_nm_v2: OK (omogeneo)"
else
  echo "gate_nm_v2: FALLITO"; FAIL=1
fi
...
print(f"identita': {diffa:.1f} vs D_AB {dab:.1f} -> scarto {diffa-dab:+.1f} (banda {BANDA}+eps {EPS}; eps=ingresso field_rmw_fast fino al kill-switch, tetto 4)")
