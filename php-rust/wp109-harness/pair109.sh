#!/bin/bash
# pair109.sh — S-100: collaudo WordPress full+media stessa-sera + coppia peak,
# versione BIMODALE del contratto di modo (A-KL-101-6). USO: pair109.sh <off|on>
...
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp109-harness"
...
}
{
  ...
  echo "epoch=$(date +%s)"
} > "$OUT/pair109.identity"
"$GUARD" backup-wipe >> "$OUT/progress.txt" 2>&1 || { echo "rc=4 guard-backup" > "$OUT/pair109.done"; exit 4; }
cd "$WPDEV" || exit 2
...
step "pair109 modo=$MODE DONE gate_void=$GATE_VOID"
echo "rc=$GATE_VOID modo=$MODE $(date +%T)" > "$OUT/pair109.done"
...
{ echo "pair109-ratios modo=$MODE: rapporti dai .time (output macchina, non prosa)"
  echo "grade=VERDICT  # derivazione meccanica riproducibile dai raw"
  echo "formato=ascii-nudo"
  ratios
} > "$H/pair109-ratios-$MODE.out"
