#!/bin/bash
# s109-grado-server.sh — S-109 punto 2: REGRADE del pin php-server POST-lotti
# (dde2a64d era gradato ma PRE-lotto-1/2; rinviato per NOME da S-108).
# Ricetta = s106-grado-server.sh INVARIATA salvo: harness wp109, stash
# php-server-s109 (nato da scripts/pin-server.sh s109), gamba phpr sul
# binario STASHATO phpr-s108 (=pin 3b3d25e2, perché la build server churna
# il release/).
#
# USO: s109-grado-server.sh <off|on>
#
# LETTERA A-PE-107-1 implementata:
#   - collaudo sul binario STASHATO (hash da PIN_SRV_ATTESO.txt,
#     FAIL-CLOSED); stash `php-server-s109` (ricetta axum-server);
...
ENV_TMPDIR=/tmp
ALLOWED='PATH|HOME|TMPDIR|LC_ALL|MIMALLOC_PURGE_DELAY|PHPR_REG_LOWER|S109_GRADO_REEXEC|PWD|SHLVL|_'
...
if [ "$SANE" != 1 ]; then
  ...
  exec /usr/bin/env -i \
    ...
    MIMALLOC_PURGE_DELAY=0 PHPR_REG_LOWER="$REG" S109_GRADO_REEXEC=1 \
    /bin/bash "$0" ${@+"$@"}
fi
unset S109_GRADO_REEXEC

# ---- pin e strumenti ----
REPO="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust"
H="$REPO/wp109-harness"
...
PHPSRV="/Volumes/Extreme Pro/Claude/phpr-old-target/release/php-server-s109"
PHPR="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s108"  # S-109: gamba phpr sul binario STASHATO del pin (la build server churna il release/)
...
mkdir -p "$OUT"
step() { echo "== $(date +%H:%M:%S) [S109-GRADO-$MODE] $1" | tee -a "$OUT/progress.txt"; }
FAILS=0
VOIDS=0
...
if [ "$H_SRV" != "$PIN_SRV_ATTESO" ]; then
  step "REFUSE: lo stash php-server-s109 NON è il pin da gradare"; echo "rc=65 voids=1 modo=$MODE $(date +%T)" > "$OUT/grado.done"; exit 65
fi
...
trap - EXIT
restore_uploads
step "S109-GRADO-$MODE DONE fails=$FAILS voids=$VOIDS"
