#!/bin/bash
# s125-cbargs2-census.sh — ARBITRO admission forma-2 (criterio cbargs2 p.4,
# committato PRIMA del run): build census+leva2, R=2 conteggi, confronto vs
# gamba A (stesso head senza leva). Predizioni: str −1,00 ESATTO, resto 0,00.
set -u
...
VERD="$H/s125-cbargs2-verdetto.out"
fail(){ echo "$1" | tee -a "$VERD"; echo 1 > "$OUT/cbargs2-census.rc"; git -C "$REPO" checkout -- . 2>/dev/null; exit 1; }

cd "$REPO" || exit 2
git diff --quiet || fail "PRE: tree sporco — STOP"
[ "$(cat "$OUT/run.rc" 2>/dev/null)" = 0 ] || fail "baseline A mancante (run.rc)"
...
CARGO_TARGET_DIR="$CTL" SOURCE_DATE_EPOCH=0 CARGO_INCREMENTAL=0 \
  ...
  > "$OUT/build-L2.log" 2>&1 || fail "build census+leva2 FALLITA"
...
for c in empty arith prop calls str arr re; do
  "$LBIN" "$MICRO/$c.php" > "$OUT/adm-$c-L2.out" 2>&1
  diff -q "$OUT/adm-$c-L2.out" "$OUT/adm-$c-pin.out" > /dev/null \
    || fail "admission cbargs2: output DIVERGE su $c — SCARTATA"
  for r in 1 2; do
    f="$OUT/L2-$c-r$r.census"; rm -f "$f"
    PHPR_MEM_CENSUS="$f" MIMALLOC_PURGE_DELAY=0 "$LBIN" "$MICRO/$c.php" > /dev/null 2>&1
    grep -q "^gacensus " "$f" || fail "gacensus ASSENTE in L2-$c-r$r"
  done
done
...
def per_iter(side, cat):
    ...
    return per[0]
print("== s125 admission cbargs2 (forma-2): census L2 (leva) vs A (baseline stesso head) ==")
PRED = {"str": -1.00}
TOL = {"arr": 0.10}
rc = 0
for cat in ["arith", "prop", "calls", "str", "arr", "re"]:
    la, lf = per_iter("L2", cat)
    ...
    if not ok: rc = 2
    print(f"{cat:6s} alloc/it A={aa:.2f} L2={la:.2f} D={da:+.2f} | free/it A={af:.2f} L2={lf:.2f} D={df:+.2f} | pred={pred:+.2f} tol={tol:.2f} -> {'OK' if ok else 'FUORI PREDIZIONE'}")
print("ADMISSION: " + ("PASS — str a PARITA' alloc con l'oracle (2,00)" if rc == 0 else "FUORI PREDIZIONE — diagnosi PRIMA dell'A/B"))
sys.exit(rc)
PY
prc=$?
echo "$prc" > "$OUT/cbargs2-census.rc"
exit "$prc"
