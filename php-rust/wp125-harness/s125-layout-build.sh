#!/bin/bash
# s125-layout-build.sh — criterio s125-criterio-banda.md p.2: tre build P1..P3
# dallo STESSO sorgente-pin s124 + no-op FIRMATO di taglia crescente, admission
# fail-closed, stash phpr-s125-lay1..3, ripristino release al pin AL BYTE.
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin:$HOME/.cargo/bin
REPO="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust"
H="$REPO/wp125-harness"
BIN="$HOME/Claude/php-rust-output/release/phpr"
P0="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s124"
...
[ "$(shasum -a 256 "$P0" | cut -c1-16)" = "$PIN_EXP" ] || fail "PRE: stash P0 != pin — STOP"

: > "$OUT/layout-hashes.env"
V=0
...
keep = anchor + """
    // s125 keep-alive (protocollo s122): materializza l'INDIRIZZO del probe
    // (mai chiamato) — senza riferimento ld64 lo dead-strippa.
    std::hint::black_box(__phpr_layout_probe_s125 as usize);"""
...
// s125 layout probe (no-op FIRMATO, s125-criterio-banda.md p.1): mai chiamata,
// esiste solo per spostare il layout .text di questa build di misura.
#[no_mangle]
#[inline(never)]
pub extern "C" fn __phpr_layout_probe_s125() -> u64 {{
...
PY
  ...
  nm "$BIN" 2>/dev/null | grep -q "__phpr_layout_probe_s125" \
  ...
  cp "$BIN" "$STASH/phpr-s125-lay$V"
  HS=$(shasum -a 256 "$STASH/phpr-s125-lay$V" | cut -c1-16)
  [ "$HB" = "$HS" ] || fail "stash P$V hash diverso ($HB vs $HS)"
  echo "P${V}_HASH=$HB" >> "$OUT/layout-hashes.env"
  note "P$V (K=$K) = $HB CONSERVATA phpr-s125-lay$V (admission 6/6+6/6, simbolo presente)"
done
