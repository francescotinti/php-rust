#!/bin/bash
# s125-promozione.sh <cand_hash16> — gate di promozione L-HD2 forma-2 cbargs2
# (criterio s125-criterio-cbargs2.md p.7; bersaglio+guardie a verbale rc=0).
...
# test nuovi attesi) → re-build ricetta → pin-phpr.sh s125 → corpus-gate
...
VERD="$H/s125-cb2-verdetto.out"
CAND_EXP="${1:?uso: s125-promozione.sh <cand_hash16>}"
...
# Inventario vs pin s124: la leva NON dichiara test nuovi — ogni differenza = STOP.
if diff -q "$SRC/wp124-harness/promo-out/batteria-nomi.txt" "$OUT/batteria-nomi.txt" > /dev/null; then
  INV="IDENTICO al pin s124 (zero test nuovi, come dichiarato)"
else
  INV="DIVERGE (diff vs wp124 in promo-out/batteria-inv.diff)"
  diff "$SRC/wp124-harness/promo-out/batteria-nomi.txt" "$OUT/batteria-nomi.txt" > "$OUT/batteria-inv.diff" || true
fi
...
"$SRC/scripts/pin-phpr.sh" s125 > "$OUT/pin.log" 2>&1
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s125.out" 2>&1
note "promozione micro pin s125: $(grep -E '^rapporto_' "$OUT/micro-pin-s125.out" | tr '\n' ' ')"
...
if diff -q "$H/orm-baseline-failnames.txt" "$OUT/orm-failnames.txt" > /dev/null; then
  note "promozione gate ORM: fail-set per NOME == baseline (16 nomi) · $SUMM"
else
  diff "$H/orm-baseline-failnames.txt" "$OUT/orm-failnames.txt" > "$OUT/orm-failnames.diff" || true
...
"$SRC/scripts/pin-server.sh" s125 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rc"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rc): pin s125 = $H2"
exit 0
