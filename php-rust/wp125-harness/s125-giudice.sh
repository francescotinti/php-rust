#!/bin/bash
# s125-giudice.sh <TAG> <TARGET> — criterio s125-criterio-cbargs.md p.5:
# giudice v3 (S-124) con SOGLIA_LAYOUT **v2-s125** (banda POST-PATCH,
# banda-out/layout-bande-v2.txt) come unica fonte per le guardie, insieme a
# 2×spread_A e 4×quanto. Legge FAIL-CLOSED banda e TSV di s125-ab.sh.
# rc: 0=PROMOZIONE-ai-gate 1=REFUTAZIONE 2=NON-DISTINGUIBILE 3=INVALIDA.
set -u
...
BANDE="$H/banda-out/layout-bande-v2.txt"
...
[ "$(cat "$H/banda-out/band.rc" 2>/dev/null)" = 0 ] || { echo "giudice $TAG: banda v2-s125 NON verde (band.rc) — STOP (leva micro senza banda = vietata)" | tee -a "$VERD"; echo 3 > "$RC"; exit 3; }
...
print(f"VERDETTO_S125_{tag} = {label}")
print(f"RCV={rc}")
PY
...
{
  echo ""
  echo "GIUDICE S-125 $TAG bersaglio=$TARGET (rc=$R da metro-out/giudice-$TAG.rc):"
  sed '/^RCV=/d' "$OUT/giudice-$TAG.txt"
} >> "$VERD"
exit "$R"
