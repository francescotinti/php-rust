...
H="$REPO/wp176-harness"; OUT="$H/census-out"; mkdir -p "$OUT"
VERD="$H/s176-census-verdetto.out"; DONE="$OUT/census.done"; rm -f "$DONE"
...
SP="/private/tmp/phpr-s176-census"; mkdir -p "$SP"
...
grep -q "^-- bigrams X -> Sweep (all) --" "$OUT/smoke-op.txt" && grep -qE "^ *[0-9]+  [A-Za-z]+ -> Sweep$" "$OUT/smoke-op.txt" \
  && grep -qE "^ *[0-9]+  IncDecSlotJmp -> [A-Za-z]+$" "$OUT/smoke-op.txt" || { echo "rc=8 smoke: sezioni S-176 assenti o vuote (census-out/smoke-op.txt)" >> "$VERD"; fin 8; }
...
tot = 0; sw = collections.Counter(); idj = collections.Counter(); sec = None; dumps = 0
...
{ echo "== s176 CENSUS «op in place + Sweep» WP media + ORM — CONTEGGI, mai tempo (criterio s176-criterio-census.md) $(date '+%F %T') =="
