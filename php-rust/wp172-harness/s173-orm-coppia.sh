...
VERD="$H/s173-orm-coppia-verdetto.out"
...
print("coppia-s173 (user CPU; pavimenti PARZIALI phpunit --version: orm o=%.3f p=%.3f · dbal o=%.3f p=%.3f)" % (fo, fp, dfo, dfp))
...
if len(nn) == 2 and len(on) == 2:
    ...
    print("nota t18 (S-172): rapporto net t18 [7,090;7,092] da rivedere; se in S-173 resta >7,05 scatta la regola 4" + (f" — t19 min={rr[0]:.3f} >7,05: RESTA SOPRA, regola 4 SCATTA" if rr[0] > 7.05 else f" — t19 min={rr[0]:.3f} <=7,05: rientra"))
...
{ echo "== s173 coppia dbal+ORM (pin s173 MISURATO $PINM vs oracle 8.5.7; criterio s173-criterio-orm.md) =="
