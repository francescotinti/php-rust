#!/bin/bash
# s172-lancio-orm.sh — catena p.1: attende pair171-t18.done; SOLO se rc=0
# lancia s172-orm-coppia.sh (MAPPA_SP dedicato APFS). Un pair fallito NON
...
if ! grep -q '^rc=0' "$PD"; then
  echo "$(date '+%F %T') pair t18 NON rc=0 ($(cat "$PD")) — ORM NON lanciato" >> "$LOG"
  exit 5
fi
sleep 60
SPD=/private/tmp/phpr-s172-orm; mkdir -p "$SPD"
echo "$(date '+%F %T') pair rc=0 — lancio ORM (MAPPA_SP=$SPD)" >> "$LOG"
MAPPA_SP="$SPD" exec /bin/bash "$H/s172-orm-coppia.sh"
