#!/bin/bash
# s173-promozione.sh <cand_hash16> <braccio> — gate di promozione LEVA L-SL2 fetta 3
# (prop residuo: P3 peephole runtime PropGetSlot+BinarySTDst, P4 borrow unico nel
# probe sigillato quando recv==slot; nessun corpo nuovo, miss ai sentieri storici)
# — S-173, criterio wp172-harness/s173-criterio.md p.6. COPIA DICHIARATA di s172-promozione.sh
# (manifest s173-promozione-copia.diff); adattamenti: tag s173, stash di riferimento phpr-s172,
# candidato phpr-s173-f3-<braccio> (B=P3, C=P3+P4), fx-sl3 NUOVA bilaterale + fx-sl3-div invariante
# (§3.32), conferma post-pin prop-dq vs stash s172, mutante P3/P4 (s173-mutante-sl3.sh) ESEGUITO
# PRIMA di questo copione (verdetto s173-mutante3-verdetto.out rc=0 richiesto in PRE).
...
VERD="$H/s173-promo-verdetto.out"
CAND_EXP="${1:?uso: s173-promozione.sh <cand_hash16> <braccio B|C>}"; ARM="${2:?braccio promosso (B=P3, C=P3+P4)}"
REF="$STASH/phpr-s172"; REF_EXP="5f2dff7d17ebed79"
...
for f in "$H/fx-ce.php" "$H/fx-am.php" "$H/fx-af.php" "$H/fx-refl.php" \
         ...
         "$H/fx-sl1.php" "$H/fx-sl1-div.php" "$H/fx-sl2.php" "$H/fx-sl2-div.php" "$H/fx-sl3.php" "$H/fx-sl3-div.php" "$H/prop-dq.php" "$H/s173-mutante3-verdetto.out" \
         ...
         "$REF" "$STASH/phpr-s173-f3-$ARM"; do
  [ -s "$f" ] || stop "PRE: path d'ingresso MANCANTE: $f — STOP"
done
[ "$(shasum -a 256 "$REF" | cut -c1-16)" = "$REF_EXP" ] || stop "PRE: stash phpr-s172 hash != $REF_EXP — STOP"
grep -q "^ESITO rc=0" "$H/s173-mutante3-verdetto.out" || stop "PRE: mutante P3/P4 non a rc=0 (s173-mutante3-verdetto.out) — STOP"
...
"$SRC/scripts/pin-phpr.sh" s173 > "$OUT/pin.log" 2>&1
...
note "promozione corpus-gate: rc=0 — nomi==congelato (1412), CONTENUTO==golden, off-on zero (ZERO flip come atteso, fetta 3)"
...
invar(){ # $1=nome $2=file $3=marcatore("" = nessuno) $4=descrizione
  ...
  "$REF" "$f" > "$OUT/$n-s172.out" 2>&1
  "$BIN" "$f" > "$OUT/$n-pin.out" 2>&1
  [ -s "$OUT/$n-pin.out" ] || stop "gate $n: output pin VUOTO"
  [ -z "$m" ] || grep -q "$m" "$OUT/$n-pin.out" || stop "gate $n: MARCATORE $m assente"
  if diff -q "$OUT/$n-s172.out" "$OUT/$n-pin.out" > /dev/null; then
    note "promozione gate $n: pin==stash s172 BYTE-ID ($d)"
  else
    diff "$OUT/$n-s172.out" "$OUT/$n-pin.out" > "$OUT/$n.diff" || true
    stop "gate $n: pin DIVERGE dallo stash s172 (promo-out/$n.diff)"
  fi
}
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s173.out" 2>&1
note "promozione micro pin s173: $(grep -E '^rapporto_' "$OUT/micro-pin-s173.out" | tr '\n' ' ')"

# ---- conferma POST-PIN prop-dq (divergenza (5)): R=5 pin s173 vs stash s172 ----
DQ="$H/prop-dq.php"
...
note "conferma post-pin prop-dq (pin s173 vs stash s172): $CONF (attesa: segno +, D nell'intorno del D_prop del braccio promosso nell'A/B; rumore > attesa/2 ⇒ SOLO SEGNO)"
...
"$SRC/scripts/pin-server.sh" s173 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb): pin s173 = $H2"
exit 0
