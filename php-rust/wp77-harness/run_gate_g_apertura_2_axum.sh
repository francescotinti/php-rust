#!/bin/bash
# G-APERTURA-2 gate runner (WP-77.6.5.2.3 Axum mode): Two sequential HTTP requests via Axum handler
# must produce byte-identical output (tests persistent RetainSet lifecycle).
#
# Against php-server (--axum mode) serving fixtures/gate_two_reqs_same_vm.php.
...
mkdir -p "$OUTDIR"
PORT=8198
VERDICT="$OUTDIR/g_apertura_2_axum.verdict"
...
# Start php-server (AXUM mode)
( "$PHPSRV" --axum --port $PORT -t "$DOC" >/dev/null 2>"$OUTDIR/g_apertura_2_axum-srv.log" ) &
SRV=$!
sleep 2

# Make TWO sequential requests to SAME endpoint
OUT1="$OUTDIR/g_apertura_2_axum_req1.out"
OUT2="$OUTDIR/g_apertura_2_axum_req2.out"
: > "$OUT1"
: > "$OUT2"
...
if [ "$FAILS" = 0 ]; then
  echo "PASS fails=0 $(date +%F_%H:%M:%S) (gate_g_apertura_2_axum.sh)" > "$VERDICT"
  echo "== G-APERTURA-2 AXUM PASS =="; exit 0
else
  echo "FAIL fails=$FAILS" > "$VERDICT"
  echo "== G-APERTURA-2 AXUM FAIL($FAILS) =="
  exit 1
fi
