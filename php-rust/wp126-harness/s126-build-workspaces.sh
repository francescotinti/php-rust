...
( cd "$BUILD/compoff-work" && COMPOSER_CACHE_DIR="$BUILD/compoff-work/ccache" COMPOSER_HOME="$BUILD/compoff-work/chome" \
    "$ORACLE" "$GATES/composer.phar" install --no-interaction --no-security-blocking --no-progress >> "$LOG" 2>&1 ) || fail "compoff warm install"
...
( cd "$BUILD/compoff-work" && rm -rf vendor && \
  ...
  "$ORACLE" "$GATES/composer.phar" install --no-interaction --no-security-blocking >> "$LOG" 2>&1 && [ -f vendor/autoload.php ] ) || fail "compoff smoke offline"
