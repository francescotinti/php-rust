#!/bin/bash
# s122-re2-build.sh — candidato L-RE2 (HEAD + re2.patch) nel target CANONICO
# con ricetta A′; admission (parità output 6 giudici + dump INTERO byte-id,
# deroga criterio p.4); stash phpr-s122-re2; RIPRISTINO di tree e release al
# pin AL BYTE. Collaudo-nell'atto: admission fallita => niente stash.
set -u
...
git apply "$H/re2.patch" || fail "re2.patch NON applica"
SOURCE_DATE_EPOCH=0 CARGO_INCREMENTAL=0 cargo build --release > "$OUT/build-cand.log" 2>&1
brc=$?; echo "$brc" > "$OUT/build-cand.rc"
if [ "$brc" != 0 ]; then git checkout -- .; fail "build candidato rc=$brc (tree ripristinato)"; fi
HB=$(shasum -a 256 "$BIN" | cut -c1-16)
note "candidato L-RE2 = $HB (target canonico, ricetta A′; A = pin $PIN_EXP)"
...
if [ "$ADM" != 0 ]; then
  ...
  fail "admission VIOLATA — candidato NON conservato, tree ripristinato"
fi
note "admission: 6/6 parità output + 6/6 dump INTERO byte-id (rc=0 da re2-out/admission.rc)"
cp "$BIN" "$STASH/phpr-s122-re2"
HS=$(shasum -a 256 "$STASH/phpr-s122-re2" | cut -c1-16)
[ "$HB" = "$HS" ] || fail "stash candidato hash diverso ($HB vs $HS)"
note "candidato CONSERVATO: phpr-s122-re2 ($HB)"

git checkout -- . || fail "checkout ripristino fallito"
SOURCE_DATE_EPOCH=0 CARGO_INCREMENTAL=0 cargo build --release > "$OUT/build-restore.log" 2>&1
rrc=$?; echo "$rrc" > "$OUT/build-restore.rc"
[ "$rrc" = 0 ] || fail "build ripristino rc=$rrc"
H1=$(shasum -a 256 "$BIN" | cut -c1-16)
[ "$H1" = "$PIN_EXP" ] || fail "RIPRISTINO NON AL BYTE: release=$H1 != $PIN_EXP — STOP"
note "ripristino: release/phpr == pin $PIN_EXP AL BYTE; A/B su stash A=phpr-s120-re1 B=phpr-s122-re2"
echo 0 > "$OUT/build.rc"
