#!/bin/bash
# s121-ab-st1.sh smoke|full — A/B L-ST1 sotto A′ (s121-criterio-st1.md p.3-5/7):
# BERSAGLIO str (promozione: D_med >= max(4,00 pavimento REGOLE; banda-zavorra
# N=3 str 2,50; 2×spread_A; 2×quanto)); le altre cinque categorie GUARDIE a
# solo-regressione (banda-v2 + re 10,00 forma S-120); floor = mediana di 3 per
# binario; rc in FILE. Smoke: precedenza smoke<->banda del p.7 — early-stop
# SOLO se concorde <=-1,00 su TUTTE le coppie E |D_med| > banda categoria.
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin
H="$(cd -P "$(dirname -- "$0")" && pwd -P)"
M="$H/../wp97-harness/micro"
A="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s120-re1"
B="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s121-st1"
MODE="${1:?smoke|full}"
RBASE="${RBASE:-5}"
if [ "$MODE" = smoke ]; then CATS="str prop"; RBASE=2; EXTRA_MAX=0; else CATS="arith prop calls str arr re"; EXTRA_MAX=6; fi
OUT="$H/st1-out"; mkdir -p "$OUT"
VERD="$H/s122-st1-ab.out"
TSV="$OUT/st1-$MODE-runs.tsv"; : > "$TSV"
...
if [ "$MODE" = smoke ]; then
  SRC=$(python3 - "$TSV" <<'PY'
import sys
# smoke di GUARDIA (criterio p.7, precedenza DECISA PRIMA): early-stop SOLO se
# (a) D <= -1.00 su TUTTE le coppie della categoria E (b) |D_med| > banda della
# categoria (str: banda-zavorra 2,50; prop: banda-v2 3,33). Solo (a) senza (b)
# => si REGISTRA e il full giudica.
from collections import defaultdict
from statistics import median
BANDA = {"str": 2.50, "prop": 3.33}
d = defaultdict(list)
...
for c, v in d.items():
    ...
    fuori = abs(dmed) > BANDA[c]
    if conc and fuori: bad = True
    if conc and not fuori:
...
PY
  ...
  echo "smoke_rc=$SRC (0=si procede al full; 1=early-stop: concorde <=-1,00 E fuori banda -> bisezione, NIENTE full)"
  awk -F'\t' '{d=(($4-$6)/$2-($5-$7)/$2)*1e9; printf "%+.2f ", d}' "$TSV" | {
    read -r line; echo "smoke R=2 guardia (rc=$SRC, regola p.7): D per coppia = $line" >> "$VERD"
  }
  exit "$SRC"
fi
...
echo "== VERDETTO A MACCHINA (bersaglio str: soglia +max(4,00; 2,50; 2×spread_A; 2×quanto); guardie banda-v2 + re 10,00; tie 2 dec) =="
python3 - "$TSV" > "$OUT/full-verdetto.txt" <<'PY'
import sys
from statistics import median
BANDA = {"arith": 0.80, "prop": 3.33, "calls": 0.50, "arr": 6.67, "re": 10.00}
SOGLIA_STR_FLOOR = 4.00      # REGOLE §3 pavimento
BANDA_ZAVORRA_STR = 2.50     # criterio p.4: zavorra N=3, sostituisce banda-v2 7,50
rows = {}; NN = {}
for line in open(sys.argv[1]):
    c, n, i, ta, tb, fa, fb = line.split('\t')
    n, ta, tb, fa, fb = float(n), float(ta), float(tb), float(fa), float(fb)
...
for c in ["arith", "prop", "calls", "str", "arr", "re"]:
    ...
    if c == "str":
        soglia = round(max(SOGLIA_STR_FLOOR, BANDA_ZAVORRA_STR, 2*sa, 2*quanto), 2)
        ok_bersaglio = dmed >= soglia
        print(f"str: fam={len(Ds)} D_med={dmed:+.2f} segni +{pos}/{len(Ds)} spread_A_dep={sa:.2f} "
...
if not ok_guardie:
    print("VERDETTO_MICRO=GUARDIA SFONDATA: bisezione p.9 (leva a UN componente => REVERT secco)")
    print("RCV=1")
elif ok_bersaglio:
    print("VERDETTO_MICRO=BERSAGLIO str OLTRE SOGLIA + GUARDIE TENGONO (si procede ai gate di promozione)")
    print("RCV=0")
else:
...
{
  echo ""
  echo "VERDETTO A MACCHINA (s121-ab-st1.sh full, rc=$FR da st1-out/full-rc):"
  sed '/^RCV=/d' "$OUT/full-verdetto.txt"
} >> "$VERD"
exit "$FR"
