#!/bin/bash
# s129-promozione.sh <cand_hash16> — gate di promozione L-OL1-F4 prelude-gate
...
VERD="$H/s129-promo-verdetto.out"
CAND_EXP="${1:?uso: s129-promozione.sh <cand_hash16>}"
...
"$SRC/scripts/pin-phpr.sh" s129 > "$OUT/pin.log" 2>&1
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s129.out" 2>&1
note "promozione micro pin s129: $(grep -E '^rapporto_' "$OUT/micro-pin-s129.out" | tr '\n' ' ')"
...
"$SRC/scripts/pin-server.sh" s129 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb — il file rc che QUESTO script scrive): pin s129 = $H2"
exit 0
