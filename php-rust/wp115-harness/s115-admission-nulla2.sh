#!/bin/bash
# s114-admission-nulla.sh — criterio s115-criterio-nulla2.md punto 6 (PRIMA della misura).
# La zavorra non tocca il pass né la semantica: (a) dump ON {main} ×6 IDENTICI
# al pin; (b) OFF al byte (dump interi) ×6; (c) taglia run_loop (nm) + bl/br
# (otool, simbolo esatto) su entrambi i lati, a verbale (target ≈ +2 KB).
set -u
...
echo "== (a) dump ON, sezione {main}: SEI IDENTICI al pin =="
for c in $CATS; do
  ...
  if cmp -s "$OUT/cand-on-$c.main" "$OUT/pin-on-$c.main"; then
    ...
    echo "   $c: {main} ON DIVERGE (VIOLAZIONE: la zavorra non deve toccare l'emissione)"; RC=1
  fi
done
