#!/bin/bash
# s101-corpus-gate.sh — gate corpus Zend per NOME, versione BIMODALE del
...
OUT="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp101-harness/corpus-gate"
mkdir -p "$OUT"
step(){ echo "$(date +%H:%M:%S) $1" | tee -a "$OUT/progress.txt"; }
FAILS=0

leg() { # $1=off|on — modo ESPLICITO (contratto value-parsed: off=>0, on=>1)
  local mode="$1" logf="$OUT/corpus-s100-$1.log" names="$OUT/corpus-s100-$1.fails" reg
