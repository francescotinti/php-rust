#!/bin/bash
# pair101.sh — S-100: collaudo WordPress full+media stessa-sera + coppia peak,
# versione BIMODALE del contratto di modo (A-KL-101-6). USO: pair101.sh <off|on>
...
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp101-harness"
...
}
{
  ...
  echo "epoch=$(date +%s)"
} > "$OUT/pair101.identity"
"$GUARD" backup-wipe >> "$OUT/progress.txt" 2>&1 || { echo "rc=4 guard-backup" > "$OUT/pair101.done"; exit 4; }
cd "$WPDEV" || exit 2
...
step "pair101 modo=$MODE DONE gate_void=$GATE_VOID"
echo "rc=$GATE_VOID modo=$MODE $(date +%T)" > "$OUT/pair101.done"
...
{ echo "pair101-ratios modo=$MODE: rapporti dai .time (output macchina, non prosa)"
  echo "grade=VERDICT  # derivazione meccanica riproducibile dai raw"
  echo "formato=ascii-nudo"
  ratios
} > "$H/pair101-ratios-$MODE.out"
