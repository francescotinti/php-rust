...
aperta, in discesa) · ultima leva SPEDITA **S-112 (H-A2)** ·
sessioni-senza-Δ-rapporti = 1 (S-113: leva tentata e refutata dal criterio) ·
incidenti «mai collaudato»: 1 (S-106) · incidenti processo: +1 (S-113 run
non-detached, guardia uploads morsa fail-closed).
...
- **phpr pin s112 = f71abd2a16da7e71** (stash `phpr-s112`; release==pin
  ...
  conteggio NON attribuito (aperto per NOME)**. Held-out baseline 6,4·2,5·5,6
  (wp112-harness/heldout-out/).
...
1. **A/B LEVA-NULLA DI ATTRIBUZIONE LAYOUT (PREREQUISITO, promosso da S-113)**:
   binario con testo INERTE nel run_loop (~+2 KB, es. cold-branch mai preso o
   padding equivalente, admission a dump identici) vs pin, R=5 sulle 6
   categorie: la **banda-layout si MISURA** (oggi: punti raccontati +1,50 S-112
   e −5,50 S-113 su calls non toccato). Con la banda misurata si ricalibrano
   le soglie di guardia (max(2×spread; banda) al posto di 0,67 N=1).
2. **LEVA PERF** (ritmo: una per sessione, A/B eseguito qualunque verdetto).
   ...
   bundle lifecycle multi-sito (i 2 get H-P1 + il doppio borrow del set
   write_property_at + recv push) progettato per attesa > banda misurata al
   punto 1; o **(i) op-count su prop** (fusione LEGALE entro il primo helper
   sospendibile, da istruire sui dump). Vincoli: criterio PRE con soglia
   max(4; rumore; banda MISURATA) + guardie a solo-regressione + held-out su
   soglia PRE + admission bipartita + ordine §6 pieno + commit+push.
...
banda-layout DA MISURARE (punto 1: +1,50/−5,50 raccontati, mai A/B proprio) ·
delta batteria 1741/1742 per NOME · arr 4,2 vs 3,9 (tra-sere) · media voce
aperta · rumore tra-sere WP (az.2 S-110) · causa fine frontend (kpc/sudo —
azione utente; quota arith da RIMISURARE post-H-A2) · §3.16/§3.17 righe
warning · retro-A/B str s107b/s108/s109 · denti rinviati (OBS-8; fx20;
direct-bind; drop-order; hit/miss) · $z++/$z-- undef non warna · §3.13 ·
§3.12-i · §3.14 · get_gc · drift TODO.md (gh-status-sync) · (b-min)
ConcatNFetchDim solo con istruttoria sopra pavimento · (g) residui BinaryDst/
CmpJmpConst: solo se un census WP li mostra caldi.
...
MySQL wp8 con l'elenco dei database · debug/ da rimuovere se rigenerata ·
uploads sotto guardia · nessuna run in volo · disco Data ~16G (stretto:
xctrace SOLO con guardie s110/s111) · held-out baseline 6,4·2,5·5,6.
