#!/bin/bash
# s121-grado-server.sh — GRADO PIENO del pin php-server s120 (debito
# dichiarato S-120: pin al MINIMO via pin-server.sh, grado pieno dovuto).
# Ricetta = s119-grado-server.sh INVARIATA salvo: harness wp121, stash
# php-server-s120 (pin 6b82236989a4a0c4), gamba phpr sul binario STASHATO
# phpr-s120-re1 (=pin s120 885d2c64, perché la build server churna il
# release/).
#
# USO: s121-grado-server.sh <off|on>
#
# LETTERA A-PE-107-1 implementata:
#   - collaudo sul binario STASHATO (hash da PIN_SRV_ATTESO.txt,
#     FAIL-CLOSED); stash `php-server-s120` (ricetta axum-server);
...
ENV_TMPDIR=/tmp
ALLOWED='PATH|HOME|TMPDIR|LC_ALL|MIMALLOC_PURGE_DELAY|PHPR_REG_LOWER|S121_GRADO_REEXEC|PWD|SHLVL|_'
...
if [ "$SANE" != 1 ]; then
  ...
  exec /usr/bin/env -i \
    ...
    MIMALLOC_PURGE_DELAY=0 PHPR_REG_LOWER="$REG" S121_GRADO_REEXEC=1 \
    /bin/bash "$0" ${@+"$@"}
fi
unset S121_GRADO_REEXEC
...
PHPSRV="/Volumes/Extreme Pro/Claude/phpr-old-target/release/php-server-s120"
PHPR="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s120-re1"  # S-109: gamba phpr sul binario STASHATO del pin (la build server churna il release/)
...
mkdir -p "$OUT"
step() { echo "== $(date +%H:%M:%S) [S121-GRADO-$MODE] $1" | tee -a "$OUT/progress.txt"; }
FAILS=0
VOIDS=0
...
if [ "$H_SRV" != "$PIN_SRV_ATTESO" ]; then
  step "REFUSE: lo stash php-server-s120 NON è il pin da gradare"; echo "rc=65 voids=1 modo=$MODE $(date +%T)" > "$OUT/grado.done"; exit 65
fi
...
trap - EXIT
restore_uploads
step "S121-GRADO-$MODE DONE fails=$FAILS voids=$VOIDS"
