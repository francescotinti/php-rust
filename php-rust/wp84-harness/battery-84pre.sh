#!/bin/bash
# battery-84pre.sh — S-84.0: FULL battery on the consolidated pre-measure
# tree, sequential (project rule). Verdict per gate BY NAME (`OK <name>`);
# A-SK30/A-SK32: the battery REV is stamped in the summary header AND inside
# .done — battery-equivalence.sh v2 now REQUIRES both stamps (OUT is legal
# evidence only for the rev it stamps). Gate set = mirror of the manifest in
# battery-equivalence.sh (A-AH36).
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin:$HOME/.cargo/bin:$PATH
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
REPO="$(cd "$HERE/.." && pwd)"
# OUTSIDE the repo (whole-tree porcelain rule).
W="/Volumes/Extreme Pro/Claude/wp84-battery-out"
mkdir -p "$W"
FAILS=0
GIT_REV="$(git -C "$REPO" rev-parse --short HEAD)"
echo "== battery-84pre git=$GIT_REV =="
run_gate() { # <label> <cmd...>
  local label="$1"; shift
  echo "== $label =="
...
if [ "$FAILS" = 0 ]; then echo "== BATTERY-84PRE PASS (15/15) git=$GIT_REV =="; else echo "== BATTERY-84PRE FAIL($FAILS) git=$GIT_REV =="; fi
