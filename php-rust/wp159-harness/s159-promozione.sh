#!/bin/bash
# s159-promozione.sh <cand_hash16> — gate di promozione LEVA L-AM1
# (array_map plumbing 0-alloc per-elemento) — S-159. COPIA DICHIARATA
# di wp158-harness/s158-promozione.sh; DIVERGENZE DICHIARATE:
#  (1) tag s159, harness wp159; candidato f2d17f18c00a4049 (A/B R=5 in
#      s159-am1-verdetto.out: giudice arrmap VINTO D=+11,0 rumore 0,0/1,0,
#      riconc. smoke in banda 1,0<4,0, riconc. UB DENTRO il modello (12,0
#      tarato ±2,5 + rumore); BANDA SMOKE VINCOLANTE [8;22] rispettata;
#      15 guardie ok, NESSUN morso; braccio A = GEMELLO 92b0aea3 == pin AL
#      BYTE (s159-gemelloA-identita.out); candidato stashato phpr-s159-am1-B);
#  (2) inventario batteria = baseline s125 + i DUE denti dichiarati (come
#      s158): rczval + loc_dente A4 — L-AM1 non aggiunge test; salite loc
#      mod.rs 25810 / host.rs 7683 GIA' dichiarate e committate (16834fac);
#  (3) corpus: ZERO flip attesi (L-AM1 preserva la semantica per costruzione:
#      fast path solo closure anonima simple arita' 1, resto invariato) —
#      rc!=0 = STOP secco, NESSUN flip-handler;
#  (4) fixture chain INVARIATA (10 gate) + gate fx-ce (bilaterale oracle==pin
#      byte-id; byte-copia in wp159) + gate sonda-bt (pin==stash s158 byte-id:
#      contratto autoload §3.25 invariato) + gate fx-refl (pin==stash s158
#      byte-id: contratto __reflect_* L-RF2 invariato; byte-copia in wp159)
#      + gate fx-am NUOVO (bilaterale oracle==pin byte-id, 14 forme array_map
#      — fast E pieno; gia' byte-id pre-promo su oracle e braccio B);
#  (5) conferma POST-PIN: m-arrmap (R=5 pin s159 vs stash phpr-s158
#      92b0aea36573955a, direzione attesa +, D nell'intorno di +11,0
#      ± rumore+drift);
#  (6) disasm run_loop GIA' AGLI ATTI pre-promo (criterio p.8: bl A=6033
#      B=6033, Δ=0 — ab-out/disasm-am1.out);
#  (7) micro R=5 = run-micro.sh come s158 (scoreboard al pin nuovo).
set -u
...
VERD="$H/s159-promo-verdetto.out"
CAND_EXP="${1:?uso: s159-promozione.sh <cand_hash16>}"
...
"$SRC/scripts/pin-phpr.sh" s159 > "$OUT/pin.log" 2>&1
...
note "promozione corpus-gate: rc=0 — nomi==congelato (1412), CONTENUTO==golden, off-on zero (ZERO flip come atteso, L-AM1)"
...
# ---- gate sonda-bt (divergenza (4)): pin==stash s158 byte-id (§3.25 invariato) ----
"$STASH/phpr-s158" "$H/sonda-bt-autoload.php" > "$OUT/sonda-bt-s158.out" 2>&1
"$BIN" "$H/sonda-bt-autoload.php" > "$OUT/sonda-bt-pin.out" 2>&1
if diff -q "$OUT/sonda-bt-s158.out" "$OUT/sonda-bt-pin.out" > /dev/null; then
  note "promozione gate sonda-bt: pin==stash s158 BYTE-ID (contratto autoload/backtrace §3.25 invariato)"
else
  stop "gate sonda-bt: pin DIVERGE dallo stash s158 (promo-out/sonda-bt-pin.out)"
fi
# ---- gate fx-refl (divergenza (4)): pin==stash s158 byte-id (contratto L-RF2 invariato) ----
"$STASH/phpr-s158" "$H/fx-refl.php" > "$OUT/fxrefl-s158.out" 2>&1
"$BIN" "$H/fx-refl.php" > "$OUT/fxrefl-pin.out" 2>&1
if diff -q "$OUT/fxrefl-s158.out" "$OUT/fxrefl-pin.out" > /dev/null; then
  note "promozione gate fx-refl: pin==stash s158 BYTE-ID (contratto __reflect_* L-RF2 invariato)"
else
  stop "gate fx-refl: pin DIVERGE dallo stash s158 (promo-out/fxrefl-pin.out)"
fi
...
if diff -q "$OUT/fxam-oracle.out" "$OUT/fxam-pin.out" > /dev/null; then
  note "promozione gate fx-am: oracle==pin BYTE-ID (14 forme array_map, fast E pieno)"
else
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s159.out" 2>&1
note "promozione micro pin s159: $(grep -E '^rapporto_' "$OUT/micro-pin-s159.out" | tr '\n' ' ')"

# ---- conferma POST-PIN m-refl (divergenza (5)): R=5 pin vs stash s158 ----
AOLD="$STASH/phpr-s158"
[ "$(shasum -a 256 "$AOLD" | cut -c1-16)" = "92b0aea36573955a" ] || stop "stash phpr-s158 hash inatteso"
...
for i in 1 2 3 4 5; do
  if [ $((i % 2)) -eq 1 ]; then TA=$(ucpu "$AOLD" "$H/m-arrmap.php"); TB=$(ucpu "$BIN" "$H/m-arrmap.php"); else TB=$(ucpu "$BIN" "$H/m-arrmap.php"); TA=$(ucpu "$AOLD" "$H/m-arrmap.php"); fi
  printf '%s\t%s\t%s\t%s\n' "$TA" "$TB" "$FA" "$FB" >> "$CT"
done
CONF=$(python3 - "$CT" <<'PY'
import sys
...
note "conferma post-pin m-arrmap (pin s159 vs stash s158, drift-tree da dichiarare): $CONF (attesa: segno +, D nell'intorno di +11,0 ± rumore+drift)"
...
"$SRC/scripts/pin-server.sh" s159 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb): pin s159 = $H2"
exit 0
