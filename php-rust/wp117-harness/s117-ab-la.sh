#!/bin/bash
# s117-ab-la.sh smoke|full — A/B rigiudizio L-A sotto A′ (s117-criterio-la.md).
# A = phpr-s117-aprime (8135dcf8), B = phpr-s117-la. Bande = banda-v2.
...
{
  echo ""
  echo "VERDETTO SOGLIE A MACCHINA (s117-ab-la.sh full, rc=$FR da la-out/full-rc):"
  sed '/^RCV=/d' "$OUT/full-verdetto.txt"
} >> "$VERD"
exit "$FR"
