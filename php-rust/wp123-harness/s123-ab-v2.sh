#!/bin/bash
# s123-ab-v2.sh <BPATH> <BEXP8> <TAG> <R> <EXTRA_MAX> <cat...> — criterio
# s123-criterio-metro.md p.5/7: SOLA misura A/B con ordine ALTERNATO per coppia
# (dispari A->B, pari B->A), timer ucpu.py, N scalati, floor med3 per binario,
# parità output sui file scalati. TSV: cat N i tA tB fA fB ord. rc in FILE.
set -u
...
echo "== s123-ab $TAG: A=885d2c64 B=$BEXP R=$RBASE extra<=$EXTRA_MAX cats=[$CATS] ordine ALTERNATO ==" >> "$VERD"
