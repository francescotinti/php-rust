#!/bin/bash
# s123-classifica-build.sh — criterio s123-criterio-classifica.md p.3: build
# census SOLO mem-census (fusione VIVA) nel target SEPARATO + admission parità
# 7/7 vs pin stash. Tree ripristinato al byte in ogni uscita.
set -u
...
# admission: parità output 7/7 census-bin vs PIN (criterio p.3)
...
{
  echo "== s123 classifica-v2 build: census_bin=$(shasum -a 256 "$CENSUS_BIN" | cut -c1-16) (SOLO mem-census, fusione VIVA)"
