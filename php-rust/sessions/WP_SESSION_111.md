...
**In una frase**: prima di progettare la leva abbiamo congelato tre giudici
«fuori concorso»; la forma minima del dispatch raggruppato fa guadagnare
all'aritmetica appena il 3% mentre tutto il resto peggiora (e i fuori concorso
peggiorano anch'essi) — la lentezza frontend NON viene dalla dispersione degli
handler caldi, e la leva è stata ritirata al byte.
...
## Esiti secchi
1·held-out CONGELATI pre-progettazione (fdbe5c8: poly tipi-alternati/megamorfo,
err error-path, wploop forma-WP; parità 9/9, N dal solo oracle) → 2·criterio
...
(regressione ×3: NON generalizza); contro-lettura delivery arith 0,325→0,295
con tempo −3,4% COERENTE ma residuo ~0,30 coi 4 handler ADIACENTI → 6·revert
...
- ⭐⭐ **Il held-out pre-congelato morde alla prima uscita**: leva che vince
  (poco) sui giudici di progetto e perde su TUTTI i fuori-concorso =
  overfitting fotografato, non opinione.
- ⭐⭐ **La contro-lettura a contatori trasforma la refutazione in meccanismo**:
  delivery −3 punti e tempo −3,4% coerenti, MA residuo ~0,30 con handler
  adiacenti ⇒ il motore della fame frontend sta altrove (salto indiretto/fetch
  helper: ipotesi da MISURARE, non premesse).
- ⭐ **Un pre-filtro sul dispatch tassa ogni op fredda** (+11% calls per −3%
  arith): la guardia esplicita sui non-bersagli nel criterio ha deciso il
  verdetto — mai ometterla.
