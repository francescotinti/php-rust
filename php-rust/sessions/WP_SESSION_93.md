...
| B1 | **I sei siti huge NOMINATI dal vivo** (hook env-gated PHPR_HUGE_TRACE=1 nel GlobalAlloc mem-census, W=1, lldb NEGATO dal sistema — via alternativa sanzionata dalla direttiva): sei chunk di RADDOPPIO (622576 → 1245168 → 2490352 → 4980720 → 9961456 → 19922928, x2+16, somma 39223200 [sanatoria Concilio WP-95: era 39423200, errore aritmetico]) di UNA arena bumpalo — l'arena AST del parse del PRELUDIO stdlib in lower_prelude_uncached (OnceCell per-thread, da main_unit_acquire nel worker_loop). Numero FISSO riprodotto (6 dopo req1, 6 dopo req3). **UNIT_CACHE, STUBS, arene dei Module FALSIFICATI come proprietari** (nessun backtrace li attraversa). Evidenza: wp93-harness/huge-sites.out (B1 ADVISORY) | 070fabf |
...
| B3 | **LEVER-2 (mi_collect on-thread post-preludio) — EFFETTO NON RILEVATO** [sanatoria Concilio WP-95: era «REFUTATA CON MISURA», retrocessa a SCREEN R=1]: arm A/B same-binary (PHPR_PRELUDE_COLLECT), slope ctrl 18814309 vs lever 18792472 B/worker → delta 21837 SOTTO la risoluzione inter-build 38229 B/worker (verdetto legale: effetto sotto la risoluzione, pin vietato). La clausola «coerente con arena committed invariata» è RITIRATA (inferenza refutata: in release committed non scende mai su purge, prim.c:504/os.c:590-591). **Canale della leva vera quantificato PRIMA (WP-48), MAGNITUDINE R=1**: contatore PHPR_PRELUDE_STATS committato — allocated_bytes=39534144 è la CAPACITÀ (touched reale 25795552, ricostruito a residuo zero); costo CLI parity su hello 44630520 vs oracle 10093048 (4.42×, pin vietato fino a R≥3). Leva nominata: **arene PER-FILE del preludio** (predizione ex-ante ben sotto il 4.42× attuale, banda in team-leva.md, gate parità COMPLETI obbligatori) | 070fabf |
...
5. **A-SK-77 ha morso live per la seconda volta**: il bump del budget
   corpus ha invalidato la citazione storica in NEXT_SESSION — il gate
   l'ha rifiutata subito (riga emendata al valore in vigore; la storia
   del budget è FUORI dal corpus, solo il valore corrente è autorità).
...
1. ⭐⭐ **Un invariante di stat si prova con la coppia alloc/free del
   ...
   refutata. **Corollario (Concilio WP-95/Leijen)**: la ragione non era
   «il decremento non morde» ma «il decremento non è compilato» —
   MI_STAT=0 in release elide `mi_stat_free`; `malloc_huge` conta le
   NASCITE, cumulativo PER COSTRUZIONE. Leggere il codice dell'allocatore
   PRIMA di attribuire un comportamento a un contatore.
...
3. ⭐ **Un probe R=1 SCREENA, non refuta** (Concilio WP-95/Bak/Gregg): il
   delta 21837 è sotto lo spread inter-build 38229 dello stesso slope —
   l'arm A/B same-binary elimina i confondenti di build ma senza repliche
   né readback dell'arm il «delta nullo» è indistinguibile da un arm mai
   scattato. «Effetto non rilevato», mai «refutato»; il pin richiede R≥5
   interleaved + eco d'arm in banda.
