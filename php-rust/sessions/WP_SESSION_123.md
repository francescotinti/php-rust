...
calls 0,73 · str 2,89 · arr 2,49 · re 4,46** ns/iter; PAV_PIN 0,01–1,36; posizioni
NON monotone; predizioni p.8 **3/3** ⇒ confondente S-122 = ordine; layout REALE ma minore.
...
3·**L-RE2 REFUTATA e ARCHIVIATA** (smoke R=6 alternato): re D_med **−15,74**, 0/6,
soglia ±10,68 — anti-tesi mosse Caps vince; 4ª caduta alloc-removal. Reperto s122-re2.
...
[revisore SEMANTICA: fusione+drift cross-pin (v1=pin s119), attribuzione =
INFERENZA — controllo pulito ±zval-census stesso head in S-124; predizione ~0-1
mancata in grado]; **arr 4,08 ≈ 2 ZStr di chiave/lettura** (INFERENZA aritmetica)
⇒ single-alloc → ~2,04 vs oracle 2,03. v1 re 17→10 = L-RE1, non fusione.
...
- ⭐⭐ **Un census vale solo sul sentiero che il release percorre** — e un delta tra
  census di PIN diversi non isola mai un solo meccanismo (revisore: 5→3 = fusione+drift).
