#!/bin/bash
# battery-89pre.sh — S-89.0: FULL battery on the WP-89 tree, sequential
...
REPO="$(cd "$HERE/.." && pwd)"
W="/Volumes/Extreme Pro/Claude/wp89-battery-out"
mkdir -p "$W"
OUTF="$W/battery-89pre.out"
: > "$OUTF"
rm -f "$W/.done"
FAILS=0
TOTAL=0
...
att_row() { # esito=... [k=v ...]
  echo "attempt_epoch=$(date +%s) battery=89pre rev=$GIT_REV $*" >> "$ATTLEDGER"
}
...
if [ -n "$DIRTY" ]; then
  ...
  say "== BATTERY-89PRE REFUSED git=$GIT_REV =="
  att_row "esito=REFUSE reason=tree-not-porcelain"
  exit 1
fi

say "== battery-89pre git=$GIT_REV =="
run_gate() { # <label> <cmd...>
  local label="$1"; shift
  TOTAL=$((TOTAL+1))
  say "== $label =="
...
PASSN=$((TOTAL-FAILS))
if [ "$FAILS" = 0 ]; then
  say "== BATTERY-89PRE PASS ($PASSN/$TOTAL) git=$GIT_REV =="
  ...
  echo "battery=89pre rev=$GIT_REV sha256=$SHA matrix=$NEWMTX matrix_sha256=$MTX_SHA" >> "$LEDGER"
  ...
  say "== BATTERY-89PRE FAIL($FAILS/$TOTAL) git=$GIT_REV =="
  ...
  att_row "esito=FAIL fails=$FAILS/$TOTAL first_fail=${FIRSTFAIL:-unknown}"
fi
exit $FAILS
