#!/bin/bash
# s132-submicro.sh — rimisura micro-ORM sul pin s132 POST-L-LO1.
# COPIA DICHIARATA di wp131-harness/s131-submicro.sh: soli cambi = pin atteso
# s132 (6af6e4975ef8d0bf), OUT/VERD in wp132-harness. Metodo INVARIATO
...
VERD="$H/s132-submicro-verdetto.out"
...
rm -f "$OUT/micro.done"
[ "$(shasum -a 256 "$PHPR" | cut -c1-16)" = "6af6e4975ef8d0bf" ] || { echo "rc=9 pin!=s132" > "$OUT/micro.done"; exit 9; }
...
{
echo "== s132 micro-ORM POST-L-LO1 sul pin s132 (metodo s127-criterio-submicro.md INVARIATO) =="
