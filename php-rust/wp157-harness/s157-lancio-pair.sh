#!/bin/bash
# s157-lancio-pair.sh — attende la fine del job CI in volo (cargo/rustc/
# phpt-runner) poi lancia s157-pair.sh t7. Il lock misura è della SESSIONE
...
LOG="$H/pair-out/lancio-t7.log"; mkdir -p "$H/pair-out"
...
echo "$(date '+%F %T') quiete raggiunta — lancio pair t7" >> "$LOG"
exec /bin/bash "$H/s157-pair.sh" t7
