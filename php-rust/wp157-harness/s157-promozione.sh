#!/bin/bash
# s157-promozione.sh <cand_hash16> — gate di promozione LEVA L-AL1
# (miss/autoload plumbing 0-alloc in try_autoload) — S-157. COPIA DICHIARATA
# di wp156-harness/s156-promozione.sh; DIVERGENZE DICHIARATE:
#  (1) tag s157, harness wp157; candidato 76787303716acd4e (A/B R=5 in
#      s157-al1-verdetto.out: giudice missload VINTO D=+22,0 rumore 1,0/5,0,
#      riconc. smoke in banda 2,0<5,0, UB in banda; 15 guardie ok — objchurn
#      morso a 3 tick e ARBITRATO refutato a N=12M, s157-objchurn-arbitrato-
#      verdetto.out; braccio A = GEMELLO RICOSTRUITO c19079d3 (contenuto==pin,
#      s157-gemelloA-identita.out); candidato stashato phpr-s157-al1-B);
#  (2) inventario batteria = baseline s125 + DUE denti dichiarati (come
#      s156): rczval + loc_dente A4 — L-AL1 non aggiunge test; salita loc
#      mod.rs 25778 GIA' dichiarata e committata (225f0bb);
#  (3) corpus: ZERO flip attesi (L-AL1 non cambia forma dei risultati) —
#      rc!=0 = STOP secco, NESSUN flip-handler;
#  (4) fixture chain INVARIATA (10 gate) + gate fx-ce (bilaterale
#      oracle==pin byte-id; copia byte-id in wp157) + gate sonda-bt-autoload
#      AGGIUNTO (A==B gia' provato; al promo: pin==braccio-B byte-id);
#  (5) conferma POST-PIN: m-missload (R=5 pin s157 vs stash phpr-s156
#      42efea3e34feb390, direzione attesa +, D nell'intorno di +22,0
#      ± rumore+drift);
#  (6) disasm run_loop GIA' AGLI ATTI pre-promo (criterio p.8: bl A=6033
#      B=6033, Δ=0 come atteso — la leva non tocca run.rs);
#  (7) micro R=5 = run-micro.sh come s156 (scoreboard al pin nuovo).
set -u
...
VERD="$H/s157-promo-verdetto.out"
CAND_EXP="${1:?uso: s157-promozione.sh <cand_hash16>}"
...
"$SRC/scripts/pin-phpr.sh" s157 > "$OUT/pin.log" 2>&1
...
note "promozione corpus-gate: rc=0 — nomi==congelato (1412), CONTENUTO==golden, off-on zero (ZERO flip come atteso, L-AL1)"
...
# ---- gate sonda-bt-autoload AGGIUNTO (divergenza (4)): pin==braccio-B byte-id ----
"$BIN" "$H/sonda-bt-autoload.php" > "$OUT/sonda-bt-pin.out" 2>&1
if diff -q "$H/sonda-out/bt-B.txt" "$OUT/sonda-bt-pin.out" > /dev/null; then
  note "promozione gate sonda-bt: pin==braccio-B BYTE-ID (contratto autoload/backtrace §3.25 invariato)"
else
  stop "gate sonda-bt: pin DIVERGE dal braccio B (promo-out/sonda-bt-pin.out)"
fi
...
PHPR="$BIN" R=5 "$SRC/wp97-harness/micro/run-micro.sh" > "$OUT/micro-pin-s157.out" 2>&1
note "promozione micro pin s157: $(grep -E '^rapporto_' "$OUT/micro-pin-s157.out" | tr '\n' ' ')"

# ---- conferma POST-PIN m-missload (divergenza (5)): R=5 pin vs stash s156 ----
AOLD="$STASH/phpr-s156"
[ "$(shasum -a 256 "$AOLD" | cut -c1-16)" = "42efea3e34feb390" ] || stop "stash phpr-s156 hash inatteso"
...
note "conferma post-pin m-missload (pin s157 vs stash s156, drift-tree da dichiarare): $CONF (attesa: segno +, D nell'intorno di +22,0 ± rumore+drift)"
...
"$SRC/scripts/pin-server.sh" s157 > "$OUT/pin-server.log" 2>&1
...
echo 0 > "$OUT/rcb"
note "PROMOZIONE COMPLETA rc=0 (da promo-out/rcb): pin s157 = $H2"
exit 0
