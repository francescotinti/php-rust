#!/bin/bash
# s157-orm-coppia.sh — coppia dbal+ORM sul pin s156 = rimisura DOVUTA (pin
# nuovo) con attesa HD2 SOTTO-risoluzione (criterio s157-criterio-orm.md; copia
# DICHIARATA di s155-orm-coppia.sh, manifest s157-orm-copia.diff; adattamenti:
# pin s156 42efea3e34feb390, path wp157, blocco ATTESA-HD2, FIX estrazione
# summ()/names() con LC_ALL=C + grep -a (az.rev. S-155 #4, diagnosi in
# wp156-harness/s156-dbal-summ-fix.md; reperto dbal ~3921/626 vs oracle
# 3929/594 A VERBALE con questa coppia): ORM + dbal, N=2 per
...
VERD="$H/s157-orm-coppia-verdetto.out"
p(){ echo "$(date +%H:%M:%S) $1" >> "$OUT/progress.txt"; }
: > "$OUT/progress.txt"
rm -f "$OUT/rimisura.done"
[ "$(shasum -a 256 "$PHPR" | cut -c1-16)" = "42efea3e34feb390" ] || { echo "rc=9 pin!=s156" > "$OUT/rimisura.done"; exit 9; }
...
print("coppia-s157 (user CPU; pavimenti PARZIALI phpunit --version: orm o=%.3f p=%.3f · dbal o=%.3f p=%.3f)" % (fo, fp, dfo, dfp))
...
# --- ATTESA HD2 (criterio s157-criterio-orm.md p.4-5, aritmetica meccanica) ---
REF_MIN, REF_MAX = 34.30, 34.35   # phpr ORM net @ s154 (s155-orm-coppia-verdetto)
RES = 0.293                        # risoluzione KS-146-1
ATT_LO, ATT_HI = 0.01, 0.05       # attesa pre-registrata SOTTO risoluzione
nn = sorted(new_net.get("orm", []))
if len(nn) == 2:
    d_min = REF_MIN - nn[-1]; d_max = REF_MAX - nn[0]
    print(f"attesa-HD2: phpr_orm_net_new=[{nn[0]:.2f}; {nn[1]:.2f}] ref=[{REF_MIN}; {REF_MAX}] Delta=[{d_min:+.2f}; {d_max:+.2f}] s")
    if parita_rc != 0:
        print("attesa-HD2: CIFRA NULLA (parita' rotta) — nessun giudizio")
    elif d_min >= RES:
        print(f"attesa-HD2: DIREZIONE GIU' FUORI RUMORE (Delta_min {d_min:.2f} >= {RES}) — OLTRE l'attesa sotto-risoluzione 0,01-0,05: dichiarare")
    elif d_max < -RES:
        print(f"attesa-HD2: REGRESSIONE SEGNALATA (Delta_max {d_max:.2f} < -{RES}) — indagine PRIMA di ogni altra leva")
    elif d_max < RES:
        print(f"attesa-HD2: COMPATIBILE (Delta dentro il rumore ±{RES}; attesa 0,01-0,05 SOTTO risoluzione — esito dichiarato, nessuna cifra attribuita a HD2)")
    else:
        print(f"attesa-HD2: NON RISOLTA (intervallo Delta a cavallo di {RES}) — si dichiara, replica eventuale")
PY

{ echo "== s157 coppia dbal+ORM (pin s156 42efea3e vs oracle 8.5.7; criterio s157-criterio-orm.md) =="
