#!/bin/bash
# s133-ab.sh <BPATH> <BEXP8> <TAG> <R> [DSMOKE_ALLOC] [DSMOKE_DATAINS] — LEVA
# ctor resolve-once (criterio s133-criterio-ctor.md, commit PRIMA di ogni run).
# COPIA DICHIARATA di wp132-harness/s132-ab.sh coi SOLI adattamenti del
# criterio p.2-4:
#  (a) A = stash pin s132 (phpr-s132, 6af6e497);
#  (b) DUE GIUDICI: objalloc E objdatains, entrambi sopra soglia per
#      promuovere; soglia per giudice = max(4, rumore drop-1 del run,
#      SPREAD-BATCH gambe B s132 = 6.7);
#  (c) bande micro-ORM FONDATE sulle gambe B del verdetto s132-ab-lo1:
#      objchurn 13.3 / objmap 10.0 (objalloc/objdatains sono giudici);
#  (d) UB modello = 35.4 (2 x 17.7, s131-propstep p.3);
#  (e) churn dichiarato: il tree B porta anche i commenti/debug_assert az.rev.
#      S-132 (c1f8f54, zero codegen release).
...
[ "$(shasum -a 256 "$A" | cut -c1-8)" = "6af6e497" ] || { echo "A != pin s132 — STOP" | tee -a "$VERD"; echo 1 > "$RC"; exit 1; }
...
{
echo "== s133-ab $TAG: A=6af6e497(pin s132) B=$BEXP R=$R ordine ALTERNATO; giudici=objalloc+objdatains (ENTRAMBI sopra soglia); soglia=max(4, drop-1, spread-batch s132 6.7); bande fondate objchurn 13.3/objmap 10.0; UB=35.4; quiescenza rc=$QRC (file: ab-out/quiesce-$TAG.rc); rc autoritativo = ab-out/$TAG.rc =="
...
UB = 35.4            # criterio p.2: 2 x 17.7 (s131-propstep p.3)
SPREAD_BATCH = 6.7   # criterio p.3: gambe B verdetto s132-ab-lo1 (objalloc
                     # 2.96-2.98, objdatains 3.61-3.63 @ N=3e6)
rows = {}
for l in open(tsv):
    t = l.rstrip("\n").split("\t")
    rows.setdefault(t[0], []).append(t)
# SL storiche (max s123/s125, committate) per le sei; bande micro-ORM FONDATE
# sulle gambe B del verdetto s132-ab-lo1 (criterio p.4).
SL = {"arith":0.94,"prop":0.80,"calls":0.73,"str":2.89,"arr":2.49,"re":4.46}
BAND = {"objmap":10.0,"objchurn":13.3}
def med(v):
    s = sorted(v); n = len(s)
...
for cat, rs in rows.items():
    ...
    if cat in JUDGES:
        ...
        ok = delta >= thr
        print(f"GIUDICE {cat}: A={ma:.1f} B={mb:.1f} ns/iter D={delta:+.1f} soglia={thr:.1f} (rumore drop-1: A'={ra:.1f} B'={rb:.1f}; spread-batch s132={SPREAD_BATCH}) -> {'SOPRA SOGLIA' if ok else 'SOTTO SOGLIA'}")
        if cat in dsmoke:
            dd = abs(dsmoke[cat] - delta)
            print(f"riconciliazione {cat}: D_smoke={dsmoke[cat]:+.1f} D_R{R}={delta:+.1f} |diff|={dd:.1f} -> {'FUORI BANDA (>soglia)' if dd > thr else 'in banda'}")
        print(f"riconciliazione UB {cat}: D={delta:+.1f} vs UB modello={UB:.1f} -> {'FUORI BANDA (D>UB)' if delta > UB else 'dentro UB'}")
