#!/bin/bash
# s133-submicro.sh — rimisura micro-ORM sul pin s133 POST-leva-ctor.
# COPIA DICHIARATA di wp132-harness/s132-submicro.sh: soli cambi = pin atteso
# s133 (hash dal pin promosso, iniettato via PIN_S133), OUT/VERD in wp133-harness. Metodo INVARIATO
...
VERD="$H/s133-submicro-verdetto.out"
...
rm -f "$OUT/micro.done"
[ "$(shasum -a 256 "$PHPR" | cut -c1-16)" = "${PIN_S133:?PIN_S133 richiesto}" ] || { echo "rc=9 pin!=s133" > "$OUT/micro.done"; exit 9; }
...
{
echo "== s133 micro-ORM POST-leva-ctor sul pin s133 (metodo s127-criterio-submicro.md INVARIATO) =="
