#!/bin/bash
# pair99.sh — S-99.0 punto 2 (ordine Concilio WP-100): collaudo WordPress
# full+media STESSA-SERA + coppia peak. Ricetta IDENTICA a pair94.sh
# (GAP_TREND §Metodo, mai reinventata): /usr/bin/time -l, DB reset + uploads
# azzerati PRIMA di ogni run, MIMALLOC_PURGE_DELAY=0, oracle PRIMA.
# In più rispetto a pair94 (KS-GR-100-1: chiusura per NOME prima di ogni
# claim CPU): estrazione dei NOMI dei failure dai due lati e diff.
# Uploads: SEMPRE dalla guardia Gregg R7 (WP-62), mai un wipe a mano.
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin:$PATH
WPDEV="/Users/francescotinti/Claude/wpdev"
H="/Volumes/Extreme Pro/Claude/php-rust-experiment/php-rust/wp99-harness"
OUT="/Volumes/Extreme Pro/Claude/wp99-harness/pair-out"; mkdir -p "$OUT"
ORACLE=/opt/homebrew/opt/php/bin/php
PHPR="$HOME/Claude/php-rust-output/release/phpr"
GUARD="/Volumes/Extreme Pro/Claude/wp62-harness/uploads-guard.sh"
rm -f "$OUT/pair99.done"
...
"$GUARD" backup-wipe >> "$OUT/progress.txt" 2>&1 || { echo "rc=4 guard-backup" > "$OUT/pair99.done"; exit 4; }
cd "$WPDEV" || exit 2
...
# ---- chiusura per NOME (KS-GR-100-1): nomi dei failure/error dai due lati.
...
for g in media full; do
  names "$OUT/$g-oracle.txt" > "$OUT/$g-oracle.failnames"
  names "$OUT/$g-phpr.txt"   > "$OUT/$g-phpr.failnames"
...
step "pair99 DONE"
echo "rc=0 $(date +%T)" > "$OUT/pair99.done"
...
{ echo "pair99-ratios: rapporti dai .time della coppia S-99 (output macchina, non prosa)"
