#!/bin/bash
# s118-ab-treno1.sh smoke|full — A/B rigiudizio L-A sotto A′ (s118-criterio-treno1.md).
# A = phpr-s117 (pin 1656580e), B = phpr-s118-treno1 (canonico). Bande = banda-v2.
# Modifiche vs S-116: (f) guardie con pavimento 2×quanto(cat); (g) floor =
# MEDIANA di 3 per binario; tie a 2 decimali; rc in FILE; esiti appesi.
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/opt/homebrew/bin
H="$(cd -P "$(dirname -- "$0")" && pwd -P)"
M="$H/../wp97-harness/micro"
A="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s117"
B="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s118-treno1"
MODE="${1:?smoke|full}"
RBASE="${RBASE:-5}"
...
TSV="$OUT/treno1-$MODE-runs.tsv"; : > "$TSV"
...
if [ -n "$INVALID" ]; then
  echo "MISURA_INVALIDA:$INVALID — rerun (criterio p.6), NESSUN verdetto"; echo 3 > "$OUT/full-rc"
  echo "full: MISURA_INVALIDA:$INVALID (rc=3)" >> "$VERD"; exit 3
fi

echo "== VERDETTO SOGLIE A MACCHINA (banda-v2; guardie −max(2×spread; banda; 2×quanto); tie 2 dec) =="
python3 - "$TSV" > "$OUT/full-verdetto.txt" <<'PY'
import sys
from statistics import median
BANDA = {"arith": 0.80, "prop": 3.33, "calls": 0.50, "str": 7.50, "arr": 6.67, "re": 0.00}
rows = {}; NN = {}
for line in open(sys.argv[1]):
    c, n, i, ta, tb, fa, fb = line.split('\t')
    n, ta, tb, fa, fb = float(n), float(ta), float(tb), float(fa), float(fb)
...
for c in ["arith", "prop", "calls", "str", "arr", "re"]:
    ...
    if c == "prop":
        ...
        ok = ok and p
        print(f"prop: fam={len(Ds)} D_med={dmed:+.2f} segni +{pos}/{len(Ds)} spread_A_dep={sa:.2f} "
              f"soglia_promozione={soglia:.2f} -> {'PASS' if p else 'FAIL'} (tie: >= = PASS)")
    else:
...
print(f"VERDETTO_MICRO={'PROMUOVIBILE (resta held-out)' if ok else 'NON promuovibile'}")
...
{
  echo ""
  echo "VERDETTO SOGLIE A MACCHINA (s118-ab-treno1.sh full, rc=$FR da treno-out/full-rc):"
  sed '/^RCV=/d' "$OUT/full-verdetto.txt"
} >> "$VERD"
exit "$FR"
