#!/bin/bash
# s118-grado-server.sh — S-118 criterio R3 gate 2: GRADO del pin php-server
...
# php-server-s118, gamba phpr sul binario STASHATO phpr-s117 (=pin 1656580e,
# perché la build server churna il release/).
#
# USO: s118-grado-server.sh <off|on>
#
# LETTERA A-PE-107-1 implementata:
#   - collaudo sul binario STASHATO (hash da PIN_SRV_ATTESO.txt,
#     FAIL-CLOSED); stash `php-server-s118` (ricetta axum-server);
...
ENV_TMPDIR=/tmp
ALLOWED='PATH|HOME|TMPDIR|LC_ALL|MIMALLOC_PURGE_DELAY|PHPR_REG_LOWER|S118_GRADO_REEXEC|PWD|SHLVL|_'
...
if [ "$SANE" != 1 ]; then
  ...
  exec /usr/bin/env -i \
    ...
    MIMALLOC_PURGE_DELAY=0 PHPR_REG_LOWER="$REG" S118_GRADO_REEXEC=1 \
    /bin/bash "$0" ${@+"$@"}
fi
unset S118_GRADO_REEXEC
...
PHPSRV="/Volumes/Extreme Pro/Claude/phpr-old-target/release/php-server-s118"
PHPR="/Volumes/Extreme Pro/Claude/phpr-old-target/release/phpr-s117"  # S-109: gamba phpr sul binario STASHATO del pin (la build server churna il release/)
...
mkdir -p "$OUT"
step() { echo "== $(date +%H:%M:%S) [S118-GRADO-$MODE] $1" | tee -a "$OUT/progress.txt"; }
FAILS=0
VOIDS=0
...
if [ "$H_SRV" != "$PIN_SRV_ATTESO" ]; then
  step "REFUSE: lo stash php-server-s118 NON è il pin da gradare"; echo "rc=65 voids=1 modo=$MODE $(date +%T)" > "$OUT/grado.done"; exit 65
fi
...
trap - EXIT
restore_uploads
step "S118-GRADO-$MODE DONE fails=$FAILS voids=$VOIDS"
