#!/bin/bash
# s118-heldout-treno1.sh — criterio s118-criterio-treno1.md (held-out, baseline = pin s117 R=5, bande A. N=2) (modifica (h)):
# baseline e banda dalla pipeline A′ (N=2), spread=max(pin,cand) ENTRAMBI
# pubblicati, tie: = limite -> ok. rc in treno-out/heldout.rc; esiti appesi.
set -u
...
RC=0
: > "$OUT/ho-esiti.txt"
for tripla in "poly 9.45 0.01" "err 2.97 0.06" "wploop 6.41 0.08"; do
...
{
  echo ""
  echo "HELD-OUT (s118-heldout-treno1.sh, rc=$RC da treno-out/heldout.rc; baseline+banda A′ N=2; spread=max(pin,cand) pubblicati):"
